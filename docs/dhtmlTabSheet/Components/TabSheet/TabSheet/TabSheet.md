...
Компонент TabSheet реализован
 классом [TabSheet](../../../Classes/TabSheet/TabSheet/TabSheet.htm).


## Пример


Компонент TabSheet может иметь
 следующий вид:
...
[Пример создания компонента TabSheet](TabSheet_Example.htm)


[JSON объект компонента TabSheet](TabSheet_JSON.htm)


Нумерация строк и столбцов таблицы начинается с 0. Столбцы в программном
 коде обозначаются так же, как строки - с помощью индексов, а не по их
 именам («A», «B», «C» и т. д.).
...
В справке и при вызове методов сначала указывается индекс строки, а
 затем индекс столбца (например, в методах [TabSheet.coord](../../../Classes/TabSheet/TabSheet/TabSheet.coord.htm),
 [TabSheet.getCell](../../../Classes/TabSheet/TabSheet/TabSheet.getCell.htm),
 за исключением метода [TabSheet.getRange](../../../Classes/TabSheet/TabSheet/TabSheet.getRange.htm),
 где вначале указывается индекс столбца). Так на следующем рисунке выделена
 ячейка с координатами (0, 2):
...
## Конструктор
				 ...
				 создает экземпляр компонента TabSheet.
...
[Работа
 с таблицей](../../../Classes/TabSheet.htm) | [DHTML-компоненты](dhtml.chm::/DHTML_components.htm)
