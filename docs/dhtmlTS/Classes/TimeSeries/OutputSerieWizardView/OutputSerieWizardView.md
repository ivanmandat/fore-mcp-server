...
Класс OutputSerieWizardView
 реализует мастер выбора выходного ряда.


## Комментарии


Мастер выбора выходного ряда имеет следующий вид:


![](OSWV22.PNG)


Для отображения панели необходимо на вкладке панели свойств «Ряд» раскрыть
 панель «Параметры». Панель доступна, если выбраны методы расчета «Линейная
 регрессия» или «Линейная регрессия (оценка методом инструментальных переменных)».


## Синтаксис


PP.initClass(PP.TS.Ui.OutputSerieWizardView, PP.Ui.[Control](dhtmlUi.chm::/Classes/control/control.htm),
 "OutputSerieWizardView", PP.IView);
...
## Свойства
		 ...
		 ![](../../../Property_Image.gif)
		 [CurrentState](OutputSerieWizardView.CurrentState.htm)
		 Свойство CurrentState
		 устанавливает состояние компонента мастера выбора выходного ряда.
...
## Методы
		 ...
		 ![](../../../Sub_Image.gif)
		 [refresh](OutputSerieWizardView.refresh.htm)
		 Метод refresh обновляет
		 компонент.


		 ![](../../../Sub_Image.gif)
		 [refreshAll](OutputSerieWizardView.refreshAll.htm)
		 Метод refreshAll является
		 аналогом метода [refresh](OutputSerieWizardView.refresh.htm).
...
## События
		 ...
		 ![](../../../Event_Image.gif)
		 [ControlChanged](dhtmlTS.chm::/Classes/TimeSeries/OutputSerieWizardView/OutputSerieWizardView.ControlChanged.htm)
		 Событие ControlChanged
		 наступает после изменения параметра в мастере.
