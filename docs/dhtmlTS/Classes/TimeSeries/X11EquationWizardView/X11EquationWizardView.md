...
Класс X11EquationWizardView
 реализует мастер выделения сезонности.


## Комментарии


Мастер выделения сезонности имеет следующий вид:


![](XIIEWV1.PNG)


Для отображения панели необходимо выбрать на вкладке ленты инструментов
 «Вычисления», нажать на кнопку «Сглаживание» и в раскрывающемся списке
 выбрать «Выделение сезонности( XII)», на вкладке панели свойств «Ряд»
 раскрыть панель «Параметры».
...
PP.initClass(PP.TS.Ui.X11EquationWizardView, PP.Ui.[Control](dhtmlUi.chm::/Classes/control/control.htm),
 "X11EquationWizardView");
...
## Свойства
		 ...
		 ![](../../../Sub_Image.gif)
		 [CurrentState](dhtmlTS.chm::/Classes/TimeSeries/X11EquationWizardView/X11EquationWizardView.CurrentState.htm)
		 Свойство CurrentState
		 устанавливает состояние компонента, которое определяет [модель
		 сезонности](dhtmlTS.chm::/Enums/StatSeasonalityType.htm).
...
## Методы
		 ...
		 ![](../../../Sub_Image.gif)
		 [setEquationPeriod](dhtmlTS.chm::/Classes/TimeSeries/X11EquationWizardView/X11EquationWizardView.setEquationPeriod.htm)
		 Метод setEquationPeriod управляет
		 доступностью элементов управления в мастере выделения сезонности
		 в соответствии с периодами расчёта сезонного эффекта.
