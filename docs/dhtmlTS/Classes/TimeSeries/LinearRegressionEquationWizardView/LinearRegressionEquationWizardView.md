...
Класс LinearRegressionEquationWizardView реализует мастер настройки параметров линейной регрессии.


## Синтаксис


PP.initClass(PP.TS.Ui.LinearRegressionEquationWizardView, PP.Ui.[Control](dhtmlUi.chm::/Classes/control/control.htm), "LinearRegressionEquationWizardView");


## Комментарии


Панель настройки параметров линейной регрессии имеет следующий вид:


![](LREWV1.PNG)


Для отображения панели необходимо в таблице выбрать несколько рядов, выбрать на вкладке ленты инструментов «Вычисления», нажать на кнопку «Регрессия» и в раскрывающемся списке выбрать «Линейная регрессия», на вкладке панели свойств «Ряд» раскрыть панель «Параметры».
...
## Свойства
 ...
 ![](../../../Property_Image.gif)
 [CurrentState](LinearRegressionEquationWizardView.CurrentState.htm)
 Свойство CurrentState возвращает параметры линейной регрессии.
...
## Методы
 ...
 ![](../../../Sub_Image.gif)
 [getlsArmaEnabled](LinearRegressionEquationWizardView.getIsArmaEnabled.htm)
 Метод getlsArmaEnabled определяет активность полей AR (авторегрессия) и MA (скользящее среднее).
