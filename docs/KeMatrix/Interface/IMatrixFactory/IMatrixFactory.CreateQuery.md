...
CreateQuery(Value: [IMatrix](../IMatrix/IMatrix.htm)):
 [IMatrixQuery](../IMatrixQuery/IMatrixQuery.htm);


## Параметры


Value. Матрица, для которой
 будет создан запрос.
...
Метод CreateQuery создает запрос
 на получение информации о существующих данных в указанной матрице.
...
При выполнении примера будет открыт куб и получена его матрица с данными
 ...
 После выполнения запроса будет получена отметка, которая соответствует
