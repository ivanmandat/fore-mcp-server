...
CreateAggregator(Id: String): [IMatrixAggregatorModel](../IMatrixAggregatorModel/IMatrixAggregatorModel.htm);


## Параметры


Id - идентификатор механизма агрегации, который необходимо создать. В качестве значений данного параметра допустимо использовать следующие значения:


-
BasicMatrixAggregator - основной механизм агрегации;


-
ChronologicalMatrixAggregator - хронологический механизм агрегации.


## Описание


Метод CreateAggregator осуществляет создание объекта, содержащего параметры агрегации данных выбранного метода.
...
[IMatrixAggregatorManager](IMatrixAggregatorManager.htm)
