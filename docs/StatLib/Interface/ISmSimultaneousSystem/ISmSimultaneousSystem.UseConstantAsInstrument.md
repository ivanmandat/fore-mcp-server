...
Допустимые значения:
	...
	- True. Константа используется
	 в качестве инструментальной переменной;


	- False. Константа используется
	 стандартным образом.
...
    // Задаем значения can,fra,ger,ita,jpn,us,uk
...
    // Используем константу в качестве инструментальной переменной

    simeq.UseConstantAsInstrument := True;

    eqs := simeq.Equations;

    // Задаем первое уравнение
...
    // Задаем стековые инструментальные переменные
...
    Debug.WriteLine("Первое уравнение. Модельный ряд");
...
    Debug.WriteLine("Второе уравнение. Модельный ряд");
...
В результате выполнения примера в окне консоли будут выведены модельные
 ряды для обоих уравнений.


См. также:


[ISmSimultaneousSystem](ISmSimultaneousSystem.htm)
