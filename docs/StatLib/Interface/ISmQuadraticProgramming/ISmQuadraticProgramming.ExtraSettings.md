...
ExtraSettings: [IQPExtraSettings](../IQPExtraSettings/IQPExtraSettings.htm);


## Описание


Свойство ExtraSettings возвращает
 дополнительные настройки, используемые при расчёте с помощью внешнего
 модуля решений Gurobi.


## Комментарии


Свойство актуально, если в свойстве [SolverType](ISmQuadraticProgramming.SolverType.htm)
 выбран модуль решений Gurobi.
...
Добавьте ссылку на системную сборку Stat.
		...
		    QP.ExtraSettings.SaveLogToFilePath := "c:\Files\Gurobi.log";

		    QP.ExtraSettings.SaveModelToFilePath := "c:\Files\Model.mps";
		...
		    Debug.WriteLine("Gurobi error code = " + QP.ExtraSettings.nErrorCode.ToString);

		    Debug.WriteLine("Gurobi message = " + QP.ExtraSettings.MessageString);
...
При выполнении примера будет произведён расчёт задачи квадратичного
 программирования с помощью внешнего модуля решений Gurobi. В консоль среды
 разработки будут выведены:


	- код возможной ошибки и сообщение, генерируемые модулем решений
	 Gurobi;
