...
Constraints: [IVarConstraints](../IVarConstraints/IVarConstraints.htm);
...
Добавьте ссылки на системные сборки: Cp, Stat.
	...
	    VarConstr.LowerBound := -100;

	    VarConstr.UpperBound := 100;

	    VarConstr.Lag := 1;

	    VarConstr := VarConstrs.Add;

	    VarConstr.LowerBound := -105;

	    VarConstr.UpperBound := 105;
...
После выполнения примера будут добавлены переменные, для переменной
 «x1» ,будут заданы ограничения. Также будут заданы прочие параметры системы
 нелинейных уравнений. Затем она будет рассчитана и результаты выведены
 в окно консоли.
