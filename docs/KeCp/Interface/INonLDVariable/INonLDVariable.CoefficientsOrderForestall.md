# INonLDVariable.CoefficientsOrderForestall
...
CoefficientsOrderForestall: String;


## Описание


Свойство CoefficientsOrderForestall
 определяет порядок коэффициентов для ожидаемых значений.
...
В данном примере создается следующая система нелинейных уравнений:
	...
	- уравнения, образующие систему:
		...
		- 0.3 * x1[t-1] + 0.1 * x2[t+1]
		 + u[t-1] * x1[t+1] *x2[t-1];


		- -0.2 * x1[t-1] + 0.4 *x2[t+1]
		 + (x1[t+1] * x2[t-1])/(v[t-1]+1).
...
После выполнения примера будут определены переменные, порядок их коэффициентов
 и прочие параметры системы нелинейных уравнений. Затем она будет рассчитана
 и результаты выведены в окно консоли.
