...
Для определения метода поиска решения используйте свойство [ICpNonLinearDecomposition.MethodType](ICpNonLinearDecomposition.MethodType.htm).
...
В данном примере создается следующая система нелинейных уравнений:
	...
	-
		...
		- x1[t] = 0.3 * x1[t-1] + 0.1 * x2[t+1] + u[t-1]
		 * x1[t+1] *x2[t-1];


		- x2[t] = -0.2 * x1[t-1] + 0.4 *x2[t+1] + (x1[t+1]
		 * x2[t-1])/(v[t-1]+1).
