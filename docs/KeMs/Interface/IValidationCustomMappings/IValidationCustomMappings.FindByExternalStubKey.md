...
FindByExternalStubKey(Key: Integer): [IValidationCustomMapping](../IValidationCustomMapping/IValidationCustomMapping.htm);


## Параметры


Key. Ключ внешнего источника
 данных, который участвует в сопоставлении.
...
Метод FindByExternalStubKey
 выполняет поиск сопоставления для внешнего источника данных с указанным
 ключом.
