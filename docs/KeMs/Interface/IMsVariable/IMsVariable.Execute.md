...
Execute(Settings: [IMsVariableLoadSettings](../IMsVariableLoadSettings/IMsVariableLoadSettings.htm));


## Параметры


Settings. Настройки, необходимые
 для загрузки данных в переменную с помощью формулы.
...
Метод Execute осуществляет загрузку
 данных в переменную. Используется при загрузке данных с помощью формулы.
...
    //Параметры агрегациипо уровням календаря
...
    //Уровень, значениякоторого используются при агрегации - Полугодия
...
    //Настройка уравнения, по которому будут рассчитываться данные переменной
