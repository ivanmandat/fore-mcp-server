...
Remainder(Input1: Variant; Input2: Variant): Variant;
...
    Expr.AsString := "Remainder(" + Inp_1 + ", " + Inp_2 + ")";
...
В результате выполнения примера модель будет возвращает остаток от деления
 точек первой входной переменной на соответствующие точки второй входной
 переменной.
...
Результат: рассчитан остаток от деления наблюдений показателя «Brazil|BCA» на соответствующие
...
Результат: рассчитан остаток от деления точек фактора «X1»
 на три.
...
База данных временных
 ...
 [Абсолютное
 значение](UiDw.chm::/Workbook/CalculatedSeries/Arithmetic/UiDw_cs_Abs.htm)
