...
PchA(Input: [ITimeSeries](../ITimeSeries/ITimeSeries.htm)):
 Variant;
...
Расчёт тема прироста точек переменной осуществляется по формуле: ((X[t]/X[t-1])S-1)*100, где S
 - сезонность.


Данный метод не применяется для годовой календарной динамики.
...
Для выполнения примера в репозитории предполагается наличие контейнера
 ...
 с идентификатором MODEL_D и квартальной динамикой, рассчитываемая методом
 детерминированного уравнения и содержащая хотя бы одну входную переменную.
...
В результате выполнения примера модель будет осуществлять расчёт темпа
 прироста точек первой входной переменной к предыдущему периоду с сезонной
 корректировкой.
...
Результат: рассчитан темп прироста наблюдений показателя «Brazil|BCA»
 к предыдущему периоду с сезонной корректировкой.
