...
PchY(Input: [ITimeSeries](../ITimeSeries/ITimeSeries.htm)):
 Variant;
...
Метод PchY осуществляет расчёт
 темпа прироста точек переменной к соответствующему периоду предыдущего
 года.
...
Расчёт тема прироста точек переменной осуществляется по формуле: (X[t]/X[t-N*F]-1)*100, где F - соответствующий период предыдущего
 года.
...
    Expr.AsString := "PchY(" + TermInfo.TermInnerText + ")";
...
В результате выполнения примера модель будет осуществлять расчёт темпа
 прироста точек первой входной переменной к соответствующему периоду предыдущего
 года.
...
Контейнер моделирования:
 [Редактирование регрессора/формулы](UiNav.Chm::/GUI/ExpressionEditor.htm),
 [Преобразования
