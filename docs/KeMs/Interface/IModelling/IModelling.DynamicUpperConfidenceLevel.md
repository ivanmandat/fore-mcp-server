# IModelling.DynamicUpperConfidenceLevel
...
DynamicUpperConfidenceLevel(Forecast: [IMsForecast](../IMsForecast/IMsForecast.htm)):
 Variant;
...
Метод DynamicUpperConfidenceLevel
 возвращает верхнюю динамическую доверительную границу прогнозного ряда.


## Комментарии


Для получения нижней динамической доверительной границы прогнозного
 ряда используйте метод [IModelling.DynamicLowerConfidenceLevel](IModelling.DynamicLowerConfidenceLevel.htm).
...
    Expr.AsString := "DynamicUpperConfidenceLevel((Arima(" + TermInfo.TermInnerText +
...
В результате выполнения примера модель будет возвращать значение верхней
 динамической доверительной границы для метода ARIMA.
...
DynamicUpperConfidenceLevel((Arima({ВВП|Анкоридж[t]},
 Null, "", "1", 0, Estimate) as ms.IMsForecastObject).Forecast(Null))
...
DynamicUpperConfidenceLevel((Arima(X1, Null, "",
