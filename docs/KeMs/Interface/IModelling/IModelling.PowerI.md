...
Метод PowerI возвращает результат
 возведения точек переменной в заданную степень, если все точки переменной
 и степень являются целыми числами.
...
Для возведения в степень точек переменной, содержащей не только целые
 значения, используйте метод [IModelling.Power](IModelling.Power.htm).
...
Результат: наблюдения показателя «Brazil|BCA»
 ...
 Если какой-либо из показателей содержит вещественные значения, то возникнет
 ошибка.
