...
## Синтаксис
		...
		UpperConfidenceLevel(Forecast: [IMsForecast](../IMsForecast/IMsForecast.htm)):
		 Variant;
...
Метод UpperConfidenceLevel возвращает
 верхнюю доверительную границу прогнозного ряда.


## Комментарии


Для получения нижней доверительной границы прогнозного ряда используйте
 метод [IModelling.LowerConfidenceLevel](IModelling.LowerConfidenceLevel.htm).
...
UpperConfidenceLevel((Arima({ВВП|Анкоридж[t]}, Null,
 "", "1", 0, Estimate) as ms.IMsForecastObject).Forecast(Null))


Результат: для модели ARIMA будут получены значения верхней доверительной
 границы.
