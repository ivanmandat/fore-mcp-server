...
HpfP(Input: [ITimeSeries](../ITimeSeries/ITimeSeries.htm);
...
Метод HpfP осуществляет сглаживание
...
Особенности задания параметров:
	...
	- Power. Параметр должен
	 быть положительным числом, значение по умолчанию - два.


Фильтр Ходрика-Прескотта с параметром сглаживания «лямбда» реализуется
 методом [IModelling.Hpf](IModelling.Hpf.htm).
...
В результате выполнения примера модель будет сглаживать данные первой
 входной переменной фильтром Ходрика-Прескотта на периоде с 2000 по 2015
 год.
...
Результат: данные временного ряда «Чикаго
 ...
 периоде данных. Степенной параметр сглаживания равен пяти.
