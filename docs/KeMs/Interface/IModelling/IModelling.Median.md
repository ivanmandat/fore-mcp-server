...
Если в Inputs указано больше
 одной переменной, то будет вычисляться медиана для соответствующих точек
 переменных. Например, медиана для точек за 2018 год по всем переменным.
...
В результате выполнения примера модель будет рассчитывать значения второй
 входной переменной, увеличенные значение медианы для первой входной переменной.
...
Результат: медиана соответствующих наблюдений рядов «Brazil|BCA»
 и «Peru|BCA» с учётом пустых
 значений за период с 2000 по 2010 год.
