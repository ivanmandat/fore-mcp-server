...
Метод Rand возвращает равномерно
 распределённое случайное число из диапазона [0, 1].
...
    Expr.AsString := TermInfo.TermInnerText + " * Rand";
...
В результате выполнения примера модель будет осуществлять умножение
 точек первой входной переменной на случайное число.
...
Результат: каждое наблюдение показателя «Brazil|BCA»
...
Результат: каждая точка фактора «X1»
