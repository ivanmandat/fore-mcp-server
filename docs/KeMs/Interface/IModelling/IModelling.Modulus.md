...
Modulus(Input1: Variant; Input2: Variant): Variant;
...
Метод Modulus
 возвращает остаток от целочисленного деления значений точек переменной
 на заданное число. Результат имеет такой же знак, как и делимое.
...
Методы Modulus и
 [IModelling.Mod_](IModelling.Mod_.htm)
...
    Expr.AsString := "Modulus(" + Inp_1 + ", " + Inp_2 + ")";
...
В результате выполнения примера модель будет возвращает остаток от целочисленного
