...
Значение точки, для которой находится логарифм, должно быть больше нуля.
...
В результате выполнения примера модель будет рассчитывать десятичный
 логарифм точек первой входной переменной.
...
Результат: рассчитан десятичный логарифм наблюдений показателя «Brazil|BCA».
...
Результат: рассчитан десятичный логарифм точек фактора «X1».
