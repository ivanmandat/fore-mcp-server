...
Интерфейс IMsRTransform предназначен
 для настройки параметров модели «R».
...
Модель «R» позволяет выполнять
 преобразование данных с помощью методов пакета R. Для работы с моделью
 в репозитории должна быть настроена интеграция с пакетом R. Подробнее
 о том, как настроить интеграцию вы можете узнать в разделе «[Как настроить
...
## Свойства
		 ...
		 ![](../../Property_Image.gif)
		 [Code](IMsRTransform.Code.htm)
		 Свойство Code
		 определяет выражение для расчета модели.


		 ![](../../Property_Image.gif)
		 [Inputs](IMsRTransform.Inputs.htm)
		 Свойство Inputs возвращает
		 список термов, соответствующих входным переменным модели.


		 ![](../../Property_Image.gif)
		 [Result](IMsRTransform.Result.htm)
