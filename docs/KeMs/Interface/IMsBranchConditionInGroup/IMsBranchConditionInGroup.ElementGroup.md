# IMsBranchConditionInGroup.ElementGroup
...
В результате выполнения примера для метамодели будет задан параметр,
 ...
 Условие состоит в следующем:
	...
	- если значение параметра равно шести или входит в группу элементов
	 справочника, тогда будет рассчитываться модель MODEL_1;


	- если значение параметра входит в отметку справочника, то будет
	 рассчитываться модель MODEL_2;


	- если ни одно из данных условий не выполняется, то будет рассчитываться
	 модель DEFAULT.
...
[IMsBranchConditionInGroup](IMsBranchConditionInGroup.htm)
