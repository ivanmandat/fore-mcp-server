...
Свойство Coord возвращает текущую
 координату, в которой производится расчёт.
...
            Debug.Write(Coord.Item(i).ToString + " ");
...
При запуске пользовательской задачи оптимизации в каждой точке расчёта
 ...
 выводиться индексы элементов измерений, которые формируют координату в
