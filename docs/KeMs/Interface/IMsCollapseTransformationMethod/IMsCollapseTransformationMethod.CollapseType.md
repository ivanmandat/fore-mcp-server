...
CollapseType: [MsCollapseType](../../Enums/MsCollapseType.htm);


## Описание


Свойство CollapseType определяет
 метод агрегации, который необходимо применить при преобразовании терма.
...
Добавьте ссылки на системные сборки: Cubes, Dimensions, Express, Laner,
 Metabase, Ms, Rds.
	...
	    // Добавим вычисляемый ряд с наименованием «Collapse»
...
После выполнения примера в рабочей книге будет добавлен новый ряд «Collapse»,
 отображающий данные указанного показателя. На его основе будет создан
 вычисляемый ряд, рассчитанный методом «Коллапс». Данные будут агрегированы
 с квартальной динамики на годовую. Метод агрегации - «Максимум».


См. также:


[IMsCollapseTransformationMethod](IMsCollapseTransformationMethod.htm)
