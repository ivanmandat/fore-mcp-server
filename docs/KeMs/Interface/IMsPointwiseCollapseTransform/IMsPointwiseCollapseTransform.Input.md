# IMsPointwiseCollapseTransform.Input
...
Для исходной динамики используйте свойство [IMsPointwiseCollapseTransform.InputLevel](IMsPointwiseCollapseTransform.InputLevel.htm).
...
Для выполнения примера в репозитории предполагается наличие контейнера
 ...
 должна присутствовать модель с идентификатором «MODEL_COLLAPSE_POINT»,
 рассчитываемая методом «Коллапс (поточечный
 расчет)». Также предполагается наличие базы данных временных рядов
 с идентификатором «TSDB», содержащая годовые и месячные динамики.
...
В результате выполнения примера будут изменены настройки расчета модели
 «MODEL_COLLAPSE_POINT»: изменена выходная переменная, метод расчета коллапса
