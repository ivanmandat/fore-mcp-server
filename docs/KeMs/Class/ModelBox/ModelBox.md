...
Класс ModelBox реализует компонент
 среды разработки [ModelBox](UiDevEnv.chm::/02_Components_constructor_forms/03_Components_of_the_access_to_data/ModelBox.htm),
 предназначенный для отображения панелей настройки модели контейнера моделирования.
...
## Свойства
		 ...
		 ![](../../Property_Image.gif)
		 [BorderStyle](../../Interface/IModelBox/IModelBox.BorderStyle.htm)
		 Свойство BorderStyle
		 определяет стиль границы компонента.


		 ![](../../Property_Image.gif)
		 [IsDirty](../../Interface/IModelBox/IModelBox.IsDirty.htm)
		 Свойство IsDirty
		 возвращает признак изменения модели, отображаемой компонентом.


		 ![](../../Property_Image.gif)
		 [Model](../../Interface/IModelBox/IModelBox.Model.htm)
		 Свойство Model определяет
		 компонент [UiModel](UiDevEnv.chm::/02_Components_constructor_forms/03_Components_of_the_access_to_data/UiModel.htm), который
		 будет использоваться как источник данных.


		 ![](../../Property_Image.gif)
		 [Pads](../../Interface/IModelBox/IModelBox.Pads.htm)
		 Свойство Pads возвращает
		 коллекцию панелей, используемых для настройки параметров модели.


		 ![](../../Property_Image.gif)
		 [ParametersSelectionSet](../../Interface/IModelBox/IModelBox.ParametersSelectionSet.htm)
		 Свойство ParametersSelectionSet
		 определяет отметку элементов в свободных измерениях моделируемой
		 переменной.


		 ![](../../Property_Image.gif)
		 [ResultsViewScenario](../../Interface/IModelBox/IModelBox.ResultsViewScenario.htm)
		 Свойство ResultsViewScenario
		 определяет ключ сценария, по которому рассчитывается модель.


		 ![](../../Property_Image.gif)
		 [ResultsViewSelectionSet](../../Interface/IModelBox/IModelBox.ResultsViewSelectionSet.htm)
		 Свойство ResultsViewSelectionSet
		 определяет отметку элементов в сквозных измерениях моделируемой
		 переменной.
...
## Методы
		 ...
		 ![](../../Sub_Image.gif)
		 [Apply](../../Interface/IModelBox/IModelBox.Apply.htm)
		 Метод Apply применяет
		 параметры, заданные в ModelBox,
		 к модели, которая является источником данных.


		 ![](../../Sub_Image.gif)
		 [Execute](../../Interface/IModelBox/IModelBox.Execute.htm)
		 Метод Execute
		 выполняет расчёт модели и обновляет компонент ModelBox.


		 ![](../../Sub_Image.gif)
		 [Export](../../Interface/IModelBox/IModelBox.Export.htm)
		 Метод Export экспортирует
		 модель, отображаемую в компоненте, в регламентный отчёт.


		 ![](../../Sub_Image.gif)
		 [SaveObject](../../Interface/IModelBox/IModelBox.SaveObject.htm)
		 Метод SaveObject осуществляет
		 сохранение изменений, внесённых в параметры модели, отображаемой
		 в компоненте.
...
## События
		 ...
		 ![](../../Event_Image.gif)
		 [OnEditObject](ModelBox.OnEditObject.htm)
		 Событие OnEditObject
		 наступает при открытии модели для редактирования.


		 ![](../../Event_Image.gif)
		 [OnUpdate](ModelBox.OnUpdate.htm)
