...
Интерфейс IPasswordCredentials
 предназначен для работы с учетными данными, используемыми при парольной
 аутентификации.
...
## Свойства
		 ...
		 ![](../../Property_Image.gif)
		 [Password](IPasswordCredentials.Password.htm)
		 Свойство Password определяет
		 пароль пользователя, под которым будет осуществлено подключение.


		 ![](../../Property_Image.gif)
		 [UserName](IPasswordCredentials.UserName.htm)
		 Свойство UserName определяет
		 имя пользователя, под которым будет осуществлено подключение.
