...
Parent: [IMetabaseObjectDescriptor](../IMetabaseObjectDescriptor/IMetabaseObjectDescriptor.htm);


## Описание


Свойство Parent определяет папку,
 в которую будет помещён созданный объект репозитория.


## Комментарии


В качестве места расположения может задаваться папка репозитория, либо
 один из объектов-контейнеров репозитория. В качестве объекта-контейнера
 могут задаваться следующие объекты: Репозиторий НСИ, База данных временных
 рядов, Контейнер моделирования, Сборка.


По умолчанию свойству установлено значение Null,
 при этом объект будет создан в корневой папке репозитория.
