...
AddAce(Type: [AceType](../../Enums/AceType.htm);
 Sid: [ISid](../ISid/ISid.htm); AccessMask: Integer): [IAccessControlEntry](../IAccessControlEntry/IAccessControlEntry.htm);
...
AccessMask.
 Маска доступа, которая будет установлена.


## Описание


Метод AddAce добавляет элемент
 контроля доступа.
...
Значение
 маски является 4-x байтовым двоичным числом, приведенным к десятичному
 виду. Для формирования значения маски необходимо использовать значения
...
После выполнения примера для указанного объекта будут изменены элементы
 контроля доступа. Если в элементах контроля доступа объекта имеются параметры,
