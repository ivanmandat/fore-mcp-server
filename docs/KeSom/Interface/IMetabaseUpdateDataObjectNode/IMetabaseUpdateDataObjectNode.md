...
Интерфейс IMetabaseUpdateDataObjectNode
...
Для работы с менеджером обновления используйте свойства и методы интерфейса
 [IMetabaseUpdate](../IMetabaseUpdate/IMetabaseUpdate.htm).
...
## Свойства
		 ...
		 ![](../../Property_Image.gif)
		 [AllowExistingRubricatorUpdate](IMetabaseUpdateDataObjectNode.AllowExistingRubricatorUpdate.htm)
		 Свойство AllowExistingRubricatorUpdate
		 определяет, разрешено ли обновление базы данных временных рядов
		 как объекта репозитория.


		 ![](../../Property_Image.gif)
		 [BatchMode](IMetabaseUpdateDataObjectNode.BatchMode.htm)
		 Свойство BatchMode
		 определяет способ обновления данных объекта.


		 ![](../../Property_Image.gif)
		 [FieldsOnly](IMetabaseUpdateDataObjectNode.FieldsOnly.htm)
		 Свойство FieldsOnly
		 определяет список полей/атрибутов для обновления.


		 ![](../../Property_Image.gif)
		 [IsIdentity](IMetabaseUpdateDataObjectNode.IsIdentity.htm)
		 Свойство IsIdentity
		 определяет, будут ли корректно обновлены данные в таблице, у которой
		 есть поле Identity.


		 ![](../../Property_Image.gif)
		 [IsPartialUpdate](IMetabaseUpdateDataObjectNode.IsPartialUpdate.htm)
		 Свойство IsPartialUpdate
		 возвращает признак частичного обновления объекта.


		 ![](../../Property_Image.gif)
		 [Method](IMetabaseUpdateDataObjectNode.Method.htm)
		 Свойство Method определяет
		 способ обновления объекта.


		 ![](../../Property_Image.gif)
		 [PrimaryKey](IMetabaseUpdateDataObjectNode.PrimaryKey.htm)
		 Свойство PrimaryKey
		 определяет список полей/атрибутов представляющих собой первичный
		 ключ.


		 ![](../../Property_Image.gif)
		 [PrimaryKeysOnly](IMetabaseUpdateDataObjectNode.PrimaryKeysOnly.htm)
		 Свойство PrimaryKeysOnly
		 определяет словарь первичных ключей для обновления отдельных записей
		 таблицы/элементов справочника.


		 ![](../../Property_Image.gif)
		 [ReferenceConstraintsHandling](IMetabaseUpdateDataObjectNode.ReferenceConstraintsHandling.htm)
		 Свойство ReferenceConstraintsHandling
		 определяет способ обработки ограничения целостности элементов
		 обновляемого объекта.


		 [![](../../Property_Image.gif)](IMetabaseUpdateDataObjectNode.RequestAdminCredentials.htm)
		 [RequestAdminCredentials](IMetabaseUpdateDataObjectNode.RequestAdminCredentials.htm)
		 Свойство RequestAdminCredentials
		 определяет, будут ли запрашиваться учетные данные пользователя,
		 если при установке обновления под текущим пользователем для
		 выполнения операции недостаточно прав.


		 ![](../../Property_Image.gif)
		 [UpdateRdsSequence](IMetabaseUpdateDataObjectNode.UpdateRdsSequence.htm)
		 Свойство UpdateRdsSequence
		 определяет признак обновления системной последовательности, которая
		 используется для генерации ключей элементов в справочниках НСИ.
