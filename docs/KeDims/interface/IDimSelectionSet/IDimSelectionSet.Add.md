...
Add(DimInstance: [IDimInstance](../IDimInstance/IDimInstance.htm)):
 [IDimSelection](../IDimSelection/IDimSelection.htm);


## Параметры


DimInstance - справочник,
 отметку которого необходимо добавить.


## Описание


Метод Add добавляет справочник
 в коллекцию и возвращает его отметку.
...
Для выполнения примера предполагается наличие в контейнере моделирования
 двух переменных, содержащих дополнительные измерения.
