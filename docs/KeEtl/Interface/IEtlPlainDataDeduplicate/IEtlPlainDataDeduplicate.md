...
Интерфейс IEtlPlainDataDeduplicate
 содержит свойства и методы объекта преобразования - Удаление дубликатов.


## Комментарии


При использовании в задаче ETL преобразователя удаление дубликатов будет
 игнорироваться [пакетная
 обработка данных](../IEtlTask/IEtlTask.PartialFetch.htm).
...
## Свойства
		 ...
		 ![](../../Property_Image.gif)
		 [DuplicatePlainOutput](IEtlPlainDataDeduplicate.DuplicatePlainOutput.htm)
		 Свойство DuplicatePlainOutput
		 возвращает объект, содержащий приёмник данных с дубликатами.


		 ![](../../Property_Image.gif)
		 [Expression](IEtlPlainDataDeduplicate.Expression.htm)
		 Свойство Expression
		 возвращает объект, содержащий выражение, вычисляемое на дублях.


		 ![](../../Property_Image.gif)
		 [Index](IEtlPlainDataDeduplicate.Index.htm)
		 Свойство Index возвращает
		 объект, содержащий индекс по которому осуществляется удаление
		 дубликатов.


		 ![](../../Property_Image.gif)
		 [PlainInput](IEtlPlainDataDeduplicate.PlainInput.htm)
		 Свойство PlainInput
		 возвращает объект, содержащий входящий источник данных.


		 ![](../../Property_Image.gif)
		 [PlainOutput](IEtlPlainDataDeduplicate.PlainOutput.htm)
		 Свойство PlainOutput
		 возвращает объект, содержащий исходящий источник данных.


		 ![](../../Property_Image.gif)
		 [Rule](IEtlPlainDataDeduplicate.Rule.htm)
		 Свойство Rule определяет
		 правило отбора вычисленных значений.
...
## Методы
		 ...
		 ![](../../Sub_Image.gif)
		 [Edit](IEtlPlainDataDeduplicate.Edit.htm)
		 Метод Edit создает
		 копию объекта для редактирования.


		 ![](../../Sub_Image.gif)
		 [Save](IEtlPlainDataDeduplicate.Save.htm)
