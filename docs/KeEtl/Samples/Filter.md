...
В данной статье рассмотрен пример создания и выполнения задачи ETL с
 фильтрацией данных источника.


Предполагается наличие в репозитории двух таблиц «T_Source»,
 «T_Destination», структура таблиц
 должна быть идентичной и должна содержать поле «ID».
 По значениям этого поля настраивается условие фильтрации. Также в репозитории
 ...
 источник репозитория, преобразователь «Фильтрация»
 и приёмник репозитория. Для всех объектов устанавливаются необходимые
 свойства, настраиваются связи, задается условие фильтрации:
...
[![](../opened.gif)![](../closed.gif)Пример](javascript:TextPopup(this))
	...
	    CreateWX(ETLTask, EtlConsumer, 60, 0);
	 ...
	 //Создаём преобразователь «Фильтрация»
	...
	    FilterInFields := Filter.PlainInput.Fields;  //Вход преобразователя

	    FilterOutFields := Filter.PlainOutput.Fields;  //Выход преобразователя
	...
	     //Настройка параметров фильтрации: пропустить все записи, для которых ID<30
	...
	    CreateWXLink(ETLTask, Shapes.FindById(EtlProvider.Id), Shapes.FindById(Filter.Id), Link);

	    CreateWXLink(ETLTask, Shapes.FindById(Filter.Id), Shapes.FindById(EtlConsumer.Id), Link1);
...
[Примеры](KeEtl_Sample.htm)
 | [IEtlPlainDataFilter](../Interface/IEtlPlainDataFilter/IEtlPlainDataFilter.htm)
