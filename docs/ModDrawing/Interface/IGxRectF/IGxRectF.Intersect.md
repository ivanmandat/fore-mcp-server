...
Intersect(Rect: [IGxRectF](IGxRectF.htm)):
...
Rect. Прямоугольник,
 с которым производится пересечение.
...
## Пример
	...
	    r1 := New GxRectF.Create(-10, -10, 10, 10);

	    r2 := New GxRectF.Create(0, 0, 20, 20);
...
При выполнении примера будут созданы два вещественных прямоугольника.
 Будет получен прямоугольник, являющийся результатом пересечения двух прямоугольников.
