...
Для настройки редактора ячейки используйте панель «Настройка
 отображения».


Примечание.
 Для использования оформления, применённого на листе, к таблице установите
 флажок «[Использовать
 оформление листа](Parameters.htm)» в группе параметров «Параметры»
 для типа области «Таблица».


[![](../../opened.gif)![](../../closed.gif)Для открытия
 панели «Настройка отображения»](javascript:TextPopup(this))


	Для открытия панели «Настройка
	 отображения» выполните следующие действия:


		- через панель параметров:
			...
			- Нажмите кнопку ![](../Table/BasicPropertiesButton.png) «Параметры» панели инструментов
			 и выберите тип области «Данные»/«Ячейки» в раскрывающемся
			 меню заголовка панели параметров.


			- Выберите группу параметров «Оформление»/«Формат ячеек».


			- Нажмите кнопку ![](../Work/b_params.png) «Настроить» в поле «Настройка редактора ячейки»
			 в группе настроек «Специальный
			 формат». Для ячеек группа настроек «Специальный
			 формат» располагается на вкладке «Настройки»;


		- через рабочую область:


			- Выделите ячейку или диапазон ячеек таблицы на рабочей
			 области.


			- Выполните команду контекстного меню ![](b_format.png)
			 «Оформление» или в
			 раскрывающемся меню команды ![](b_format.png) «Оформление»
			 выберите тип области данные/измерение/уровень/элемент.
...
Набор отображаемых настроек зависит от типа редактора. Допустимые типы
 редактора:
	...
	- Список возможных значений.
	 В данном поле формируется дерево возможных значений.
		...
		- для переименования элемента выполните команду «Переименовать»
		 раскрывающегося меню кнопки ![](b_value_settings.png) «Настройка»;


		- для удаления выбранного элемента нажмите кнопку ![](Delete.png) «Удалить»
...
[![](../../opened.gif)![](../../closed.gif)Поле ввода строковых
...
[![](../../opened.gif)![](../../closed.gif)Поле ввода строковых
 значений с маской](javascript:TextPopup(this))
...
Выбрать можно несколько вариантов. Идентификатор
 и ключ будут указаны в скобках;
		...
		- Атрибут справочника, управляющий
		 ...
		 передавать значения атрибута «Ключ».
	...
	Выбранный атрибут должен иметь уникальный
	 индекс. Это необходимо для восстановления отметки;
		...
		- Способ отметки.
		 В раскрывающемся списке выберите способ отметки элементов справочника:
	...
	Для инвертирования отметки элемента щёлкните
	 по нему.
	...
	Щелчок с удержанием клавиши CTRL приводит
	 к установке/снятию отметки элемента;
			...
			- None.
			 Тип отметки не задан, по умолчанию используется способ отметки
			 «MultiSelect»;
	 ...
	 По умолчанию для всех уровней используется атрибут «Наименование».
			...
			- для
			 отображения пользовательского наименования уровней укажите:
				...
				- Пользовательское
				 наименование. В поле укажите наименование, которое
				 будет отображаться для всех уровней в раскрывающемся списке;


				- Вставить атрибут.
				 В раскрывающемся меню кнопки ![](edit.png) «Настройка» списке выберите
				 атрибут, который будет добавлен в наименование.


	На листе формы ввода работа с раскрывающемся списком доступна:
		...
		- с помощью клавиш клавиатуры:
			...
			- для открытия списка используйте клавишу SPACE;


			- для закрытия списка используйте клавиши ENTER/TAB/ESC;


			- для перемещения по списку используйте клавиши UP/DOWN;


			- для выделения/снятия отметки элемента списка используйте
			 клавишу SPACE.


[![](../../opened.gif)![](../../closed.gif)Редактор времени](javascript:TextPopup(this))
...
[![](../../opened.gif)![](../../closed.gif)Редактор даты](javascript:TextPopup(this))
...
[![](../../opened.gif)![](../../closed.gif)Редактор даты
 и времени](javascript:TextPopup(this))
...
[![](../../opened.gif)![](../../closed.gif)Редактор десятичных
 значений](javascript:TextPopup(this))
	...
	Задайте параметры редактора:
		...
		- Точность значения.
		 Определите количество разрядов числа. Данное значение должно быть
		 в диапазоне допустимых значений [0, 28]. По умолчанию в числе
		 используется 14 цифр после запятой;
		...
		- Значение по умолчанию.
		 ...
		 значений [-179228162514264337593543950335; 179228162514264337593543950335];
		...
		- Значение может быть пустым.
		 Установите флажок для отображения пустого значения в редакторе
		 значений. Если флажок снят, то отображается значение, заданное
		 по умолчанию.


[![](../../opened.gif)![](../../closed.gif)Редактор идентификаторов](javascript:TextPopup(this))
...
[![](../../opened.gif)![](../../closed.gif)Редактор логических
 значений](javascript:TextPopup(this))
...
[![](../../opened.gif)![](../../closed.gif)Редактор целых
 значений](javascript:TextPopup(this))
...
## Подключение календаря с настраиваемым диапазоном дат


Для удобства работы с календарным справочником в редакторе типа «[Раскрывающийся
 список справочника](Display_setting.htm#dimension)» используйте календарь с настраиваемым диапазоном
 дат.
...
Для подключения календаря в строке связи «[Настройка редактора ячейки](Format.htm#special_format)»
...
Для задания диапазона дат:
	...
	- Откройте календарь, нажав кнопку ![](b_calendar.png) «Открыть календарь»:
...
Для сброса диапазона дат нажмите кнопку ![](../Work/b_search_bar_close.png) «Очистить».
...
[Настройка оформления
 и вида отображения формы](Design.htm) | [Настройка оформления](Format.htm)
