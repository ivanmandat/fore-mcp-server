...
Класс DtMetabaseProvider реализует
 объект, осуществляющий импорт данных из таблиц репозитория.
...
Объект данного класса может применяться для прямого импорта данных из
 таблиц репозитория, а также используется при настройке объекта задачи
 Etl - [Источник
 репозитория](UiETL.chm::/Desktop/03_ETLObjects/02_Inputs/UiEtl_Inputs_Repo.htm).


## Свойства, унаследованные от [IDtMetabaseProvider](../../Interface/IDtMetabaseProvider/IDtMetabaseProvider.htm)
		 ...
		 ![](../../Property_Image.gif)
		 [Dataset](../../Interface/IDtMetabaseProvider/IDtMetabaseProvider.Dataset.htm)
		 Свойство Dataset определяет
		 реляционный объект репозитория, с которым связан данный источник
		 данных.
