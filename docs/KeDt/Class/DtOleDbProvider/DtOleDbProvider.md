...
Класс DtOleDbProvider реализует
 объект, осуществляющий импорт данных из источников, связь с которыми осуществляется
...
Объект данного класса может применяться для прямого импорта данных из
 OleDb источников, а также используется при настройке объекта задачи Etl
 - [Импорт
 из OleDb](UiETL.chm::/Desktop/03_ETLObjects/02_Inputs/UiEtl_Inputs_OleDB.htm).


## Свойства, унаследованные от [IDtOleDbProvider](../../Interface/IDtOleDbProvider/IDtOleDbProvider.htm)
		 ...
		 ![](../../Property_Image.gif)
		 [ConnectionString](../../Interface/IDtOleDbProvider/IDtOleDbProvider.ConnectionString.htm)


		 Свойство ConnectionString
		 определяет строку соединения, используемую для подключения к источнику
		 данных.


		 ![](../../Property_Image.gif)
		 [Query](../../Interface/IDtOleDbProvider/IDtOleDbProvider.Query.htm)


		 Свойство Query определяет
		 запрос к таблицам, хранящимся в источнике данных.
