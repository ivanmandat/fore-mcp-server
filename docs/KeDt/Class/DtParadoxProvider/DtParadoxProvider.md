...
Класс DtParadoxProvider реализует
 объект, осуществляющий импорт данных из таблиц в формате Paradox.
...
Объект данного класса может применяться для прямого импорта данных из
 таблиц Paradox, а также используется при настройке объекта задачи Etl
 - [Импорт
 из Paradox](UiETL.chm::/Desktop/03_ETLObjects/02_Inputs/UiEtl_Inputs_Paradox.htm).


## Свойства, унаследованные от [IDtParadoxProvider](../../Interface/IDtParadoxProvider/IDtParadoxProvider.htm)
		 ...
		 ![](../../Property_Image.gif)
		 [DriverVersion](../../Interface/IDtParadoxProvider/IDtParadoxProvider.DriverVersion.htm)
		 ...
		 ![](../../Property_Image.gif)
		 [Path](../../Interface/IDtParadoxProvider/IDtParadoxProvider.Path.htm)


		 Свойство Path определяет
		 путь к каталогу, в котором располагаются таблицы Paradox, из которых
		 будут импортироваться данные.


		 ![](../../Property_Image.gif)
		 [Query](../../Interface/IDtParadoxProvider/IDtParadoxProvider.Query.htm)
