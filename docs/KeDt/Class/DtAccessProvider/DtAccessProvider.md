...
Класс DtAccessProvider реализует
 объект, осуществляющий импорт данных из таблиц, хранящихся в базах данных
 Microsoft Access.
...
Объект данного класса может применяться для прямого импорта данных из
 таблиц Microsoft Access, а также используется при настройке объекта задачи
 Etl - [Импорт
 из Access](UiETL.chm::/Desktop/03_ETLObjects/02_Inputs/UIetl_Inputs_Access.htm).


## Свойства, унаследованные от [IDtAccessProvider](../../Interface/IDtAccessProvider/IDtAccessProvider.htm)
		 ...
		 ![](../../Property_Image.gif)
		 [File](../../Interface/IDtAccessProvider/IDtAccessProvider.File.htm)


		 Свойство File определяет
		 наименование файла базы данных Access, из которой будет производиться
		 импорт данных.


		 ![](../../Property_Image.gif)
		 [Query](../../Interface/IDtAccessProvider/IDtAccessProvider.Query.htm)


		 Свойство Query определяет
		 запрос к таблицам, хранящимся в базе данных Access.
