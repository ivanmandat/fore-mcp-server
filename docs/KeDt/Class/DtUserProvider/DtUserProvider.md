...
Класс DtUserProvider реализует
 объект, осуществляющий импорт данных на основе алгоритма, реализованного
...
Объект данного класса может применяться для прямого импорта массива
 данных с помощью пользовательского алгоритма. Пользовательский алгоритм
 реализуется в методе [IDtRecordsetProvider.Fetch](../../Interface/IDtRecordsetProvider/IDtRecordsetProvider.Fetch.htm),
 который должен быть переопределен в пользовательском классе.


Также используется при настройке объекта задачи Etl - [Источник пользователя](UiETL.chm::/Desktop/03_ETLObjects/02_Inputs/Uietl_Inputs_User.htm).


## Свойства, унаследованные от [IDtUserProvider](../../Interface/IDtUserProvider/IDtUserProvider.htm)
		 ...
		 ![](../../Property_Image.gif)
		 [ForeClass](../../Interface/IDtUserProvider/IDtUserProvider.ForeClass.htm)
		 Свойство ForeClass
		 определяет наименование класса, реализующего источник пользователя.


		 ![](../../Property_Image.gif)
		 [ForeModule](../../Interface/IDtUserProvider/IDtUserProvider.ForeModule.htm)
		 Свойство ForeModule
		 определяет модуль, в котором содержится класс, реализующий источник
		 пользователя.


		 ![](../../Property_Image.gif)
		 [UserData](../../Interface/IDtUserProvider/IDtUserProvider.UserData.htm)
		 Свойство UserData определяет
		 дополнительные настройки в формате XML для пользовательского источника
		 данных.
