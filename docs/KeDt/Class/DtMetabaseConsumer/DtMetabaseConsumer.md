...
Класс DtMetabaseConsumer реализует
 объект, осуществляющий экспорт данных в таблицы репозитория.
...
Объект данного класса может применяться для прямого экспорта массива
 данных в таблицы репозитория, а также используется при настройке объекта
 задачи Etl - [Приёмник
 репозитория](UiETL.chm::/Desktop/03_ETLObjects/03_Outputs/Repositoriy/UiEtl_Outputs_Repo.htm).


## Свойства, унаследованные от [IDtMetabaseConsumer](../../Interface/IDtMetabaseConsumer/IDtMetabaseConsumer.htm)
		 ...
		 ![](../../Property_Image.gif)
		 [CatchAddRecordException](../../Interface/IDtMetabaseConsumer/IDtMetabaseConsumer.CatchAddRecordException.htm)
		 Свойство CatchAddRecordException
		 определяет поведение при возникновении ошибки при добавлении записи
		 в буфер.


		 ![](../../Property_Image.gif)
		 [Dataset](../../Interface/IDtMetabaseConsumer/IDtMetabaseConsumer.Dataset.htm)
		 Свойство Dataset определяет
		 реляционный объект репозитория, с которым связан данный приёмник
		 данных.


		 ![](../../Property_Image.gif)
		 [KeyFieldNames](../../Interface/IDtMetabaseConsumer/IDtMetabaseConsumer.KeyFieldNames.htm)
		 Свойство KeyFieldNames
		 определяет список ключевых полей, по значениям которых будет осуществляться
		 поиск обновляемых записей.
