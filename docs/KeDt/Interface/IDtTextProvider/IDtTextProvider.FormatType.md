...
FormatType: [DtTextFormatType](../../Enums/DtTextFormatType.htm);
...
По умолчанию используется формат с разделителями.
...
После выполнения примера в задаче ETL будет создан объект «Импорт из
 текста». Источник будет импортировать данные из указанного текстового
 ...
 одно поле будет отводиться 15 позиций. Из первой строки файла будут импортироваться
 наименования полей.
