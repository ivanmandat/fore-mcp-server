...
[ICtrlGroupConditions](ModForms.chm::/Interface/ICtrlGroupConditions/ICtrlGroupConditions.htm)


[CtrlGroupConditions](ModForms.chm::/Class/CtrlGroupConditions/CtrlGroupConditions.htm)


## Описание


Компонент CtrlGroupConditions
 используется для группировки элементов и работы с ними.


## Работа с компонентом


Компонент реализован по принципу работы компонента [ListView](ListView.htm).
 Элемент компонента состоит из строки и столбцов, которые описывают элемент.
 Группа элементов может состоять из набора сгруппированных элементов или
 одного элемента.


По умолчанию в компоненте заданы два столбца, недоступные для удаления.
 В первом столбце отображаются флажки для отметки элементов и сформированные
 группы элементов. Во втором столбце отображаются выражения, в которых
 определяются операторы объединения элементов в группе. Для получения столбцов
 компонента используйте свойство [IListView.Columns](ModForms.chm::/Interface/IListView/IListView.Columns.htm).


Примечание.
 Первый элемент не содержит выражения.


Для работы с компонентом:


	- Добавьте компонент CtrlGroupConditions
	 на [форму](../../01_Development_Environment/02_Work_in_Development_Environment/DevEnv_Object/DevEnv_Form.htm).


	- Добавьте столбцы для описания элементов компонента с помощью
	 окна «[Редактор
	 колонок](ModForms.chm::/Interface/IListView/IListView.Columns.htm)».


	- Добавьте элементы с описанием по заданным столбцам с помощью
	 свойств и методов компонента.


	- Отметьте элементы для группировки с помощью флажков в первом
	 столбце.


	- Сгруппируйте или разгруппируйте группы элементов при необходимости.


После выполнения действий в компоненте будут содержаться заданные элементы
 или группы элементов с описанием по столбцам.


## Пример


Пример компонента CtrlGroupConditions:


![](CtrlGroupConditions_Sample.png)


## Свойства, унаследованные от [ICtrlGroupConditions](ModForms.chm::/Interface/ICtrlGroupConditions/ICtrlGroupConditions.htm)
		 ...
		 ![](../../Property_Image.gif)
		 [Groups](ModForms.chm::/Interface/ICtrlGroupConditions/ICtrlGroupConditions.Groups.htm)
		 Свойство Groups определяет
		 коллекцию групп элементов.


		 ![](../../Property_Image.gif)
		 [SelectedRows](ModForms.chm::/Interface/ICtrlGroupConditions/ICtrlGroupConditions.SelectedRows.htm)
		 Свойство SelectedRows
		 определяет массив отмеченных элементов.


## Свойства, унаследованные от [IListView](ModForms.chm::/interface/ilistview/ilistview.htm)
...
## Методы, унаследованные от [ICtrlGroupConditions](ModForms.chm::/Interface/ICtrlGroupConditions/ICtrlGroupConditions.htm)
		 ...
		 ![](../../Sub_Image.gif)
		 [AddRow](ModForms.chm::/Interface/ICtrlGroupConditions/ICtrlGroupConditions.AddRow.htm)
		 Метод AddRow добавляет
		 элемент.


		 ![](../../Sub_Image.gif)
		 [CanGroup](ModForms.chm::/Interface/ICtrlGroupConditions/ICtrlGroupConditions.CanGroup.htm)
		 Метод CanGroup определяет
		 возможность группировки элементов.


		 ![](../../Sub_Image.gif)
		 [CanUngroup](ModForms.chm::/Interface/ICtrlGroupConditions/ICtrlGroupConditions.CanUngroup.htm)
		 Метод CanUngroup определяет
		 возможность разгруппировки группы элементов.


		 ![](../../Sub_Image.gif)
		 [GetControl](ModForms.chm::/Interface/ICtrlGroupConditions/ICtrlGroupConditions.GetControl.htm)
		 Метод GetControl возвращает
		 визуальный компонент, добавленный в ячейку элемента.


		 ![](../../Sub_Image.gif)
		 [Group](ModForms.chm::/Interface/ICtrlGroupConditions/ICtrlGroupConditions.Group.htm)
		 Метод Group группирует
		 отмеченные элементы.


		 ![](../../Sub_Image.gif)
		 [GroupByIndexes](ModForms.chm::/Interface/ICtrlGroupConditions/ICtrlGroupConditions.GroupByIndexes.htm)
		 Метод GroupByIndexes
		 группирует элементы по индексам.


		 ![](../../Sub_Image.gif)
		 [InsertRow](ModForms.chm::/Interface/ICtrlGroupConditions/ICtrlGroupConditions.InsertRow.htm)
		 Метод InsertRow добавляет
		 элемент по указанному индексу.


		 ![](../../Sub_Image.gif)
		 [Ungroup](ModForms.chm::/Interface/ICtrlGroupConditions/ICtrlGroupConditions.Ungroup.htm)
		 Метод Ungroup разгруппировывает
		 отмеченную группу элементов.


		 ![](../../Sub_Image.gif)
		 [UngroupByIndexes](ModForms.chm::/Interface/ICtrlGroupConditions/ICtrlGroupConditions.UngroupByIndexes.htm)
...
## События, унаследованные от [ListView](ModForms.chm::/Class/ListView/ListView.htm)
