...
## Иерархия наследования
 ...
           [IButton](ModForms.chm::/Interface/IButton/IButton.htm)


           [IImageButton](ModForms.chm::/Interface/IImageButton/IImageButton.htm)


           [ImageButton](ModForms.chm::/Class/ImageButton/ImageButton.htm)


## Описание


Компонент ImageButton представляет
 собой стандартный компонент [Button](../01_Standart_Components/Button.htm)
 с расширенными возможностями по настройке внешнего вида.


При вставке нового компонента на форму он имеет вид, аналогичный обычной
 [кнопке](../01_Standart_Components/Button.htm). Для использования
 пиктограмм вместе с текстом необходимо в свойстве [Images](ModForms.chm::/Interface/IImageButton/IImageButton.Images.htm)
 определить компонент [ImageList](ImageList.htm) ([GlobalImageList](GlobalImageList.htm)),
 содержащий коллекцию пиктограмм. После этого для каждого режима кнопки
 в свойстве [ImageIndex](ModForms.chm::/Interface/IImageButtonLook/IImageButtonLook.ImageIndex.htm)
 можно указать индекс необходимой пиктограммы.


Также для компонента ImageButton
 реализована возможность установки фонового изображения. Для его установки
 в каждом режиме предусмотрено свойство [Background](ModForms.chm::/Interface/IImageButtonLook/IImageButtonLook.Background.htm).
 При обращении к данному свойству в инспекторе объектов вызывается редактор
 фонового изображения компонента ImageButton.
 Данный редактор также вызывается при выборе для компонента пункта контекстного
 меню «Редактор изображений»:


![](ImageButton_Image_Editor.gif)


В окне редактора необходимо выбрать состояние кнопки, для которого будет
 установлено фоновое изображение. При нажатии кнопки «Загрузить»
 будет выведен стандартный диалог выбора файла, который будет использоваться
 в качестве фонового изображения. В список доступных изображений входят
 файлы в форматах: *.bmp;*.jpg;*.jpeg;*.jpe;*.jfif;*.gif;*.png;*.tif;*.tiff.
 Если размеры выбранной картинки больше размеров окна редактирования, то
 изображение будет автоматически смасштабировано.


В группе «Фиксировать область»
 можно настроить области изображения, которые не будут масштабироваться
 при отображении. Для этого фоновое изображение условно разбивается на
 9 частей:


![](ImageButton_TransformImage.gif)
...
Флажок «Отображать области»
 управляет отображением красных линий, отделяющих фиксированные области
 изображения от масштабированных. Фиксированные области отображаются в
 том виде, в каком они содержатся в исходном файле. Вся остальная область
 изображения масштабируется в соответствии с размерами компонента ImageButton. Размеры фиксированной
 области задаются в пикселях и ограничены размерами загруженного изображения.


Флажок  «Растянуть изображение»
 позволяет растянуть загруженное изображение на всю область окна редактирования.


Кнопка  «Скопировать из»
 позволяет применить для текущего режима настройки из другого режима кнопки.


Кнопка  «Применить к» позволяет
 установить настройки текущего режима для какого-либо другого режима либо
 для всех остальных режимов.


Для настройки фонового изображения с отступами через ядро реализован
 специальный класс [GxRectangleTransformImage](ModDrawing.chm::/Class/GxRectangleTransformImage/GxRectangleTransformImage.htm).
 Параметры фиксированных отступов задаются в свойстве [Margins](ModDrawing.chm::/Interface/IGxRectangleTransformImage/IGxRectangleTransformImage.Margins.htm).


## Пример


![](ImageButton_Sample.gif)
...
## Свойства компонента ImageButton
		 ...
		 ![](../../Property_Image.gif)
		 [Alignment](ModForms.chm::/Interface/IImageButton/IImageButton.Alignment.htm)
		 Свойство Alignment
		 определяет расположение текста внутри поля компонента.


		 ![](../../Property_Image.gif)
		 [AllowAllUp](ModForms.chm::/Interface/IImageButton/IImageButton.AllowAllUp.htm)
		 Свойство AllowAllUp
		 определяет, могут ли все кнопки, составляющие группу, находиться
		 одновременно в состоянии "отпущена".


		 ![](../../Property_Image.gif)
		 [AllowDown](ModForms.chm::/Interface/IImageButton/IImageButton.AllowDown.htm)
		 Свойство AllowDown
		 определяет признак, будет ли кнопка после первого нажатия оставаться
		 в этом состоянии.
		 ...
		 ![](../../Property_Image.gif)
		 [BorderStyle](ModForms.chm::/Interface/IImageButton/IImageButton.BorderStyle.htm)
		 ...
		 ![](../../Property_Image.gif)
		 [CancelButton](ModForms.chm::/Interface/IButton/IButton.CancelButton.htm)
		 Свойство CancelButton
		 определяет, будет ли обрабатываться событие OnClick при нажатии
		 клавиши ESC.


		 ![](../../Property_Image.gif)
		 [Center](ModForms.chm::/Interface/IImageButton/IImageButton.Center.htm)
		 Свойство Center определяет
		 признак расположения картинки в центре кнопки.
		 ...
		 ![](../../Property_Image.gif)
		 [DefaultButton](ModForms.chm::/Interface/IButton/IButton.DefaultButton.htm)
		 Свойство DefaultButton
		 определяет, что нажатие пользователем клавиши ENTER будет эквивалентно
		 нажатию на данную кнопку.


		 ![](../../Property_Image.gif)
		 [DisabledLook](ModForms.chm::/Interface/IImageButton/IImageButton.DisabledLook.htm)
		 Свойство DisabledLook
		 определяет внешний вид кнопки (изображение и цвет текста надписи),
		 который она примет, находясь в состоянии "недоступна".


		 ![](../../Property_Image.gif)
		 [Down](ModForms.chm::/Interface/IImageButton/IImageButton.Down.htm)
		 Свойство Down определяет
		 в каком состоянии находится кнопка.


		 ![](../../Property_Image.gif)
		 [DrawFocus](ModForms.chm::/Interface/IImageButton/IImageButton.DrawFocus.htm)
		 Свойство DrawFocus
		 определяет признак отображения рамки фокуса вдоль границы компонента.
		 ...
		 ![](../../Property_Image.gif)
		 [GroupIndex](ModForms.chm::/Interface/IImageButton/IImageButton.GroupIndex.htm)
		 Свойство GroupIndex
		 определяет индекс группы, в которую входит компонент.
		 ...
		 ![](../../Property_Image.gif)
		 [HoverLook](ModForms.chm::/Interface/IImageButton/IImageButton.HoverLook.htm)
		 Свойство HoverLook
		 определяет внешний вид кнопки (изображение и цвет текста надписи),
		 который она примет при прохождении над ней курсора мыши.


		 ![](../../Property_Image.gif)
		 [ImageLayout](ModForms.chm::/Interface/IImageButton/IImageButton.ImageLayout.htm)
		 Свойство ImageLayout
		 определяет способ расположения пиктограммы относительно текста
		 компонента.


		 ![](../../Property_Image.gif)
		 [Images](ModForms.chm::/Interface/IImageButton/IImageButton.Images.htm)
		 Свойство Images определяет
		 наименование компонента ImageList,
		 изображения которого будут использоваться в компоненте.


		 ![](../../Property_Image.gif)
		 [Layout](ModForms.chm::/Interface/IImageButton/IImageButton.Layout.htm)
		 Свойство Layout определяет
		 выравнивание текста по вертикали внутри поля компонента.
		 ...
		 ![](../../Property_Image.gif)
		 [Margin](ModForms.chm::/Interface/IImageButton/IImageButton.Margin.htm)
		 Свойство Margin определяет
		 отступ от границы до группы элементов компонента.


		 ![](../../Property_Image.gif)
		 [Margins](ModForms.chm::/Interface/IImageButton/IImageButton.Margins.htm)
		 Свойство Margins возвращает
		 параметры границ области текста компонента относительно границ
		 самого компонента.


		 ![](../../Property_Image.gif)
		 [ModalResult](ModForms.chm::/Interface/IButton/IButton.ModalResult.htm)
		 Свойство ModalResult
		 определяет результат, возвращаемый после нажатия на кнопку на
		 модальной форме при закрытии этой формы.
		 ...
		 ![](../../Property_Image.gif)
		 [NormalLook](ModForms.chm::/Interface/IImageButton/IImageButton.NormalLook.htm)
		 Свойство NormalLook
		 возвращает настройки внешнего вида кнопки (изображение и цвет
		 текста надписи), который она примет в обычном не нажатом состоянии.
		 ...
		 ![](../../Property_Image.gif)
		 [PressedLook](ModForms.chm::/Interface/IImageButton/IImageButton.PressedLook.htm)
		 Свойство PressedLook
		 определяет внешний вид кнопки (изображение и цвет текста надписи),
		 который она примет в нажатом состоянии.


		 ![](../../Property_Image.gif)
		 [Proportional](ModForms.chm::/Interface/IImageButton/IImageButton.Proportional.htm)
		 Свойство Proportional
		 определяет признак соблюдения пропорций изображения в окне компонента.
		 ...
		 ![](../../Property_Image.gif)
		 [Spacing](ModForms.chm::/Interface/IImageButton/IImageButton.Spacing.htm)
		 Свойство Spacing определяет
		 интервал между пиктограммой и текстом компонента.


		 ![](../../Property_Image.gif)
		 [Stretch](ModForms.chm::/Interface/IImageButton/IImageButton.Stretch.htm)
		 Свойство Stretch определяет,
		 будут ли размеры изображения изменяться таким образом, чтобы изображение
		 целиком поместилось в окне элемента.
		 ...
		 ![](../../Property_Image.gif)
		 [TransparentBackground](ModForms.chm::/Interface/IImageButton/IImageButton.TransparentBackground.htm)
		 Свойство TransparentBackground
		 определяет признак использования прозрачного фона кнопки.


		 ![](../../Property_Image.gif)
		 [Trimming](ModForms.chm::/Interface/IImageButton/IImageButton.Trimming.htm)
		 Свойство Trimming определяет
		 режим отсечения текста.
		 ...
		 ![](../../Property_Image.gif)
		 [WordWrap](ModForms.chm::/Interface/IImageButton/IImageButton.WordWrap.htm)
		 Свойство WordWrap определяет,
		 будут ли переноситься слова, превышающие длину компонента, на
		 новую строку.
