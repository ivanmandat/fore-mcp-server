...
## Иерархия наследования
 ...
           [IWebBrowserBox](ModForms.chm::/Interface/IWebBrowserBox/IWebBrowserBox.htm)


           [WebBrowserBox](ModForms.chm::/Class/WebBrowserBox/WebBrowserBox.htm)


## Описание


Компонент WebBrowserBox предназначен
 для загрузки и отображения веб-страниц.


## Работа с компонентом


Для загрузки какой-либо веб-страницы используйте метод [Navigate](ModForms.chm::/Interface/IWebBrowserBox/IWebBrowserBox.Navigate.htm).
 Также, если имеется html-код страницы, то его можно задать в свойстве
 [DocumentText](ModForms.chm::/Interface/IWebBrowserBox/IWebBrowserBox.DocumentText.htm).
 Управление загруженной страницей осуществляется посредством свойств и
 методов объекта WebBrowser, лежащего
 в основе компонента WebBrowserBox.
 Также поддерживаются свойства/методы интерфейса IWebBrowser2, используемые
 при работе с экземплярами браузера Internet Explorer. Вся работа осуществляется
 через свойство [OleObject](ModForms.chm::/Interface/IWebBrowserBox/IWebBrowserBox.OleObject.htm).
 Описание WebBrowser/IWebBrowser2
 представлено в MSDN.


При навигации в компоненте могут возникнуть различного рода ошибки не
 позволяющие корректно отобразить веб-страницы. Это связано с тем, что
 по умолчанию WebBrowser использует
 ядро устаревшего браузера Internet Explorer 7.0. Для решения проблемы
 можно воспользоваться методом, описание которого представлено в статье
 [Internet
 Feature Controls (MSDN)](http://msdn.microsoft.com/en-us/library/ee330730.aspx). В подразделе реестра [HKEY_LOCAL_MACHINE
 (или HKEY_CURRENT_USER)\Software\Microsoft\Internet Explorer\Main\FeatureControl\FEATURE_BROWSER_EMULATION]
 создайте параметр типа REG_DWORD с наименованием запускаемого приложения (в случае
 запуска «Форсайт. Аналитическая платформа» - параметр с наименованием
 «studio.exe» или «pp.exe») и установите для него значение, соответствующее
 необходимой версии Internet Explorer (значения приведены в таблице по
 указанной ссылке). После этого в компоненте будет использоваться эмуляция
 более новой версии браузера Internet Explorer, что позволит избежать возникновения
 ряда ошибок, связанных с различием версий браузера.


## Пример


![](WebBrowserBox_Sample.gif)


## Свойства компонента WebBrowserBox
		 ...
		 ![](../../Property_Image.gif)
		 [BorderStyle](ModForms.chm::/Interface/IWebBrowserBox/IWebBrowserBox.BorderStyle.htm)
		 ...
		 определяет HTML код страницы, загруженной в компоненте.
		 ...
		 ![](../../Property_Image.gif)
		 [OleObject](ModForms.chm::/Interface/IWebBrowserBox/IWebBrowserBox.OleObject.htm)
		 Свойство OleObject
...
## Методы компонента WebBrowserBox
		 ...
		 ![](../../Sub_Image.gif)
		 [Navigate](ModForms.chm::/Interface/IWebBrowserBox/IWebBrowserBox.Navigate.htm)
		 Метод Navigate осуществляет
		 загрузку в компонент веб-страницу, адрес которой передается в
		 качестве входного параметра.


		 ![](../../Sub_Image.gif)
		 [Refresh](ModForms.chm::/Interface/IWebBrowserBox/IWebBrowserBox.Refresh.htm)
...
## События компонента WebBrowserBox
		 ...
		 ![](../../Event_Image.gif)
		 [OnBeforeNavigate2](ModForms.chm::/Class/WebBrowserBox/WebBrowserBox.OnBeforeNavigate2.htm)
		 ...
		 ![](../../Event_Image.gif)
		 [OnDocumentComplete](ModForms.chm::/Class/WebBrowserBox/WebBrowserBox.OnDocumentComplete.htm)
		 ...
		 ![](../../Event_Image.gif)
		 [OnNavigateComplete2](ModForms.chm::/Class/WebBrowserBox/WebBrowserBox.OnNavigateComplete2.htm)
