...
## Иерархия наследования
 ...
           [IWebInput](WebForms.chm::/Interface/IWebInput/IWebInput.htm)


           [WebInput](WebForms.chm::/Class/WebInput/WebInput.htm)


## Описание


Компонент Input реализует поле
 ввода данных.
...
Компонент имеет различный набор свойств, доступных в режиме дизайнера
 и режиме выполнения веб-формы.
	 ...
	 События
			...
			- placeholder.
			 Временный текст, который отображается в компоненте, если в
			 нём отсутствует значение;
		...
		Компонент имеет событие onTextChanged,
		 в котором может отслеживаться вводимый текст.
