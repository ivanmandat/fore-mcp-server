...
Свойство Primary определяет признак первичного индекса.


## Комментарии


Для первичного индекса автоматически устанавливается признак [уникальности](ITableIndex.Unique.htm). Первичный ключ создается по полям, не допускающим [пустые значения](../ITableField/ITableField.Mandatory.htm). При добавлении полей в коллекцию первичного индекса, данный признак для них будет выставлен автоматически.
