...
Поля в списке разделяются знаком ";". Искомые значения будут
 браться из буфера.
...
После выполнения примера в таблице будут обновлены записи, значения
 первого поля которых, больше 100. Поиск существующих записей будет производиться
 по значениям первого поля. При обновлении будет изменено значение третьего
 поля с учетом значений предыдущих двух полей.
