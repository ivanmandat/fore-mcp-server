...
Интерфейс IDalTriggers содержит
 свойства и методы для работы с курсором, который содержит системную информацию
 о триггерах таблиц, хранящихся в БД. При работе с сервером БД существует
 ограничение на количество одновременно открытых курсоров, поэтому необходимо
 своевременно закрывать неиспользуемые курсоры, используя метод [Close](../IDalCursor/IDalCursor.Close.htm).
