# Создание шага «Расчёт»: Веб-приложение
...
# Создание шага «Расчёт»
	...
	Для создания шага по проведению вычислений:


		- Выделите этап процесса или
		 шаг этапа и выберите шаг ![](b_create_calculation.png) «Расчёт» из раскрывающегося списка
		 кнопки ![](b_create_def.png) «Ввод
		 данных». Кнопка выбора шага имеет вид последнего выбранного
		 шага.


		- В рабочей области выделите
		 созданный шаг «Расчёт»
		 и нажмите кнопку ![](../b_settings.png) «Настройки»
		 на панели инструментов. Будет открыта панель параметров «Расчёт» для настройки шага.
	...
	Настройка шага «Расчёт»
	 состоит из следующих действий:
		...
		- [Настройка базовых
		 свойств и параметров описания шага](Calculation.htm#base_settings).
		...
		- [Настройка действий, выполняемых
		 при наступлении события](Calculation.htm#events).
...
Выбрать можно несколько вариантов. Идентификатор и ключ будут указаны
 в скобках;
		...
		- Способ исполнения.
		 Установите переключатель в положение «Ручной»
		 или «Автоматический»:
	...
	По умолчанию для шага установлен автоматический
	 ...
	 обозначаются пиктограммой ![](../auto_calc.png).
	 Изменить способ исполнения можно в настройках шага, для этого выделите
	 шаг в рабочей области при настройке бизнес-процесса и нажмите кнопку
	 ![](../b_settings.png) «Настройки» на панели инструментов.
...
## Настройка связи параметров алгоритма расчёта
	...
	Для запуска алгоритма расчёта с теми же параметрами, что и шаг процесса,
	 выполните настройку связи параметров алгоритма расчёта и процесса
	 на вкладке «Связь» панели
	 параметров шага:


	![](calculation_connection.png)


	Примечание.
	 Если алгоритм расчёта не содержит параметров, то список отображается
	 пустым.


	Для каждого статичного поля, в котором отображается наименование
	 параметра алгоритма расчёта, выберите из раскрывающегося списка соответствующий
	 ...
	 и алгоритма расчёта, должны иметь идентичную структуру или [связаны
	 при помощи атрибутов](UiNavObj.chm::/reference_book/Master_RDS_reference_book/Link.htm). Если параметр не выбран,
	 то алгоритм расчёта открывается со значением параметра, заданным в
	 алгоритме расчёта по умолчанию.


	Выберите из раскрывающегося списка «Тип
	 отметки» один из типов отметки:


	Примечание.
	 Список «Тип отметки» доступен
	 для параметров алгоритма расчёта, у которых установлен справочник
	 в качестве параметра процесса.


		- Исходная. Включает
		 элементы только первого уровня иерархии;
	...
	![](scheme_web.png)
		...
		- По функции. Используется
		 схема отметки, сформированная с помощью функции Fore, например:
	...
	Примечание.
	 После выбора модуля, укажите метод: Function <имя функции>(selection:
	 IDimSelection): IDimSelection.


	Для установки требуемых значений параметров, с которыми будет открываться
	 алгоритм расчёта, выполните следующие действия:


		- Нажмите кнопку ![](b_fix.png)«Зафиксировать значения алгоритма расчёта».
		 В раскрывающемся списке будут доступны элементы справочника-параметра
		 алгоритма расчёта.


		- Выберите значение параметра алгоритма расчёта, прикрепленного
		 к шагу процесса. Доступна множественная отметка.


	Если значение параметра не выбрано, то
	 в алгоритме расчёта отметка будет отсутствовать.
