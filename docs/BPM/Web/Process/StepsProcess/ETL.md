# Создание шага «Задача ETL»: Веб-приложение
...
# Создание шага «Задача ETL»
	...
	Для создания шага по выполнению задачи ETL:


		- Выделите этап процесса или
		 шаг этапа и выберите шаг ![](b_create_etl.png) «Задача
		 ETL» из раскрывающегося списка кнопки ![](b_create_def.png) «Ввод данных». Кнопка выбора
		 шага имеет вид последнего выбранного шага.


		- В рабочей области выделите
		 созданный шаг «Задача ETL»
		 и нажмите кнопку ![](../b_settings.png) «Настройки»
		 на панели инструментов. Будет открыта панель параметров «Задача ETL» для настройки шага;
	...
	Настройка шага «Задача ETL»
	 состоит из следующих действий:
		...
		- [Настройка базовых свойств
		 и параметров описания шага](ETL.htm#base_settings).
		...
		- [Настройка действий, выполняемых
		 при наступлении события](ETL.htm#events).
...
## Настройка базовых свойств и параметров
	...
	На панели параметров для шага «Задача
	 ETL» на вкладке «Настройки» после
	 определения [базовых
	 свойств шага](CreateStepsProcess.htm#steps_process_firs_page) задайте следующий параметр:


		- Задача ETL. Поле
		 обязательно для заполнения. Выберите из раскрывающегося списка
		 задачу ETL.
...
## Настройка связи параметров задачи ETL
	...
	Для выполнения задачи ETL с теми же параметрами, что и запущенный
	 шаг процесса, выполните настройку связи задачи ETL и процесса на вкладке
	...
	Примечание.
	 Если задача ETL не содержит параметров, то список отображается пустым.


	На панели параметров для шага «Задача
	 ETL» на вкладке «Связь»
	 для каждого статичного поля, в котором отображается наименование параметра
	 задачи ETL, выберите из раскрывающегося списка соответствующий параметр
	 процесса. В качестве параметра может быть установлен только один справочник
	 или один атрибут справочника. Справочники, выбранные в качестве [параметров процесса](../Params_control.htm) и задачи ETL,
	 ...
	 то задача ETL открывается со значением параметра, заданным в задаче
	 ETL по умолчанию.


	Для установки требуемых значений параметров, с которыми будет открываться
	 задача ETL, выполните следующие действия:


		- Нажмите кнопку ![](b_fix.png)«Зафиксировать значения параметра задачи
		 ETL». В раскрывающемся списке будут доступны элементы справочника-параметра
		 задачи ETL.


		- Выберите значение параметра задачи ETL, прикрепленной к
		 шагу процесса. Доступна множественная отметка.


	Если значение параметра не выбрано, то
	 в задаче ETL отметка будет отсутствовать.
