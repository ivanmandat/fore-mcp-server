...
Свойство CreateDerivedSeries
 определяет, добавлять ли в качестве дочерних вычисленные ряды («Модельный
 ряд», «Остатки» и т.д.).
...
Допустимые значения:
	...
	-  True. Добавлять
	 дочерние ряды. Значение по умолчанию;
...
После выполнения примера для первого ряда рабочей книги будет задано
 преобразование, но дочерние ряды добавлены не будут. Пример вычисляемого
 ряда с добавленными дочерними рядами (слева) и без добавления дочерних
 рядов (справа):


![](ILanerCalculateSerie_CreateDerivedSeries.gif)
