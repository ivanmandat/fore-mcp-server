...
Класс ColorSectorsPanel реализует панель мастера спидометра для настройки секторов.


## Комментарии


Панель «Секторы» выглядит следующим образом:


![](ColorSectorsPanel.gif)


Панель можно получить из мастера спидометра при помощи метода [GaugeMaster](GaugeMaster/GaugeMaster.htm).getItem(3).


## Синтаксис


PP.initClass(PP.Ui.ColorSectorsPanel,  PP.Ui.[MasterPanel](dhtmlUi.chm::/Classes/MasterPanel/MasterPanel.htm), "PP.Ui.ColorSectorsPanel", PP.Ui.INotifySettingsChanged);
