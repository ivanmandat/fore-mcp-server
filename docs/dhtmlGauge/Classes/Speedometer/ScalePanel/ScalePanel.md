...
Класс ScalePanel реализует панель мастера спидометра для настройки шкалы.


## Комментарии


Панель «Шкала» выглядит следующим образом:


![](../ScalePanel.gif)


Панель можно получить из мастера спидометра при помощи метода [GaugeMaster](../GaugeMaster/GaugeMaster.htm).getItem(2).


## Синтаксис


PP.initClass(PP.Ui.ScalePanel,  PP.Ui.[MasterPanel](dhtmlUi.chm::/Classes/MasterPanel/MasterPanel.htm), "PP.Ui.ScalePanel", PP.Ui.INotifySettingsChanged);
...
## События
 ...
 ![](../../../Event_Image.gif)
 [CurrentScaleChanged](ScalePanel.CurrentScaleChanged.htm)
 Событие CurrentScaleChanged наступает при смене текущей шкалы спидометра.
