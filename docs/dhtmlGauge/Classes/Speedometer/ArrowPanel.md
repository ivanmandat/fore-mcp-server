...
Класс ArrowPanel представляет
 собой панель для настройки стрелки.


## Комментарии


Панель «Стрелка» выглядит следующим образом:


![](ArrowPanel.gif)


Панель можно получить из мастера спидометра при помощи метода [GaugeMaster](GaugeMaster/GaugeMaster.htm).master.getItem(4).


Панель активна, если на вкладке «Шкала» выбрана текущая шкала, для которой
 настроена стрелка.
...
PP.initClass(PP.Ui.ArrowPanel,  PP.Ui.[MasterPanel](dhtmlUi.chm::/Classes/MasterPanel/MasterPanel.htm),
 "PP.Ui.ArrowPanel", PP.Ui.INotifySettingsChanged);
