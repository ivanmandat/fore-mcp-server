# SumX2MY2: Регламентный отчёт, настольное приложение

SumX2MY2: Регламентный отчёт, настольное приложение
-


# SumX2MY2


[Мастер функций](../../UiReport_Organizational_master_function.htm)
 для функции SumX2MY2 выглядит
 следующим образом:


![](SumX2MY2_m.gif)


## Синтаксис


SumX2MY2(X, Y)


## Параметры


X. Диапазон ячеек, в которых
 расположены числа первого массива;


Y. Диапазон ячеек, в которых
 расположены числа второго массива.


Примечание.
 Размерности первого и второго массива должны совпадать, иначе функция
 возвращает значение 0.


## Описание


Возвращает сумму разностей квадратов соответствующих значений в двух
 массивах.


## Пример


		 Формула
		 Результат
		 Описание


		 =SumX2MY2(B6:E6, B7:E7)
		 29,96
		 Сумма разностей квадратов соответствующих значений в диапазонах
		 ячеек от B6 до E6 и от B7 до E7. В диапазоне B6:E6 расположены
		 следующие числа: 3; 4; 3,6; -0,2; а в диапазоне B7:E7 расположены
		 следующие числа: 2; -1,4; 0,8; 1,2.


См. также:


[Мастер функций](../../UiReport_Organizational_master_function.htm)
 │ [Математические
 функции](UiReport_Func_math.htm) │ [Sum](UiReport_Func_Math_Sum.htm)
 │ [SumSq](UiReport_Func_Math_SumSq.htm)
 │ [SumX2PY2](UiReport_Func_Math_SumX2PY2.htm)
 │ [SumXMY2](UiReport_Func_Math_SumXMY2.htm)
 │ [IMath.SumX2MY2](MathLib.chm::/Interface/IMath/IMath.SumX2MY2.htm)


		Справочная
		 система на версию 10.9
		 от 18/08/2025,
		 © ООО «ФОРСАЙТ»,
