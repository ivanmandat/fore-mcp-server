# Today: Регламентный отчёт, настольное приложение

Today: Регламентный отчёт, настольное приложение
-


# Today


[Мастер функций](../../UiReport_Organizational_master_function.htm)
 для функции Today выглядит следующим
 образом:


![](Today.gif)


## Синтаксис


Today()


## Параметры


Данная функция не содержит параметров.


## Описание


Возвращает дату на текущий момент времени.


## Комментарии


Текущая дата возвращаются при каждом вычислении листа.


## Пример


		 Формула
		 Результат
		 Описание


		 =Today
		 Меняется
		 Текущая дата.


См. также:


[Мастер функций](../../UiReport_Organizational_master_function.htm)
 │ [Дата
 и время](UiReport_Func_DateTime.htm) │ [Day](UiReport_Func_DateTime_Day.htm)
 │[Hour](UiReport_Func_DateTime_Hour.htm)
 │ [Minute](UiReport_Func_DateTime_Minute.htm)
 │ [Month](UiReport_Func_DateTime_Month.htm)
 [│
 Now](UiReport_Func_DateTime_Now.htm) │ [Second](UiReport_Func_DateTime_Second.htm)
 │ [Weekday](UiReport_Func_DateTime_Weekday.htm)
 │ [Year](UiReport_Func_DateTime_Year.htm)


		Справочная
		 система на версию 10.9
		 от 18/08/2025,
		 © ООО «ФОРСАЙТ»,
