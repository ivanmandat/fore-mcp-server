# Настройка пользовательских кнопок: Регламентный отчёт, веб-приложение
...
Пользовательские кнопки отображаются на [панели
 инструментов](../../organizational_management/Starting.htm#structure_window) и предназначены для ручного выполнения Fore-модулей или
...
Доступные операции:
	...
	- [добавление
	 пользовательских кнопок](../../organizational_management/Toolbar_settings.htm#button);


	- [редактирование
	 пользовательских кнопок](../../organizational_management/Toolbar_settings.htm#edit);


	- [удаление
	 пользовательских кнопок](../../organizational_management/Toolbar_settings.htm#delete).
...
Задание настроек пользовательских кнопок выполняется с помощью соответствующих
 вкладок окна «Настройки пользовательской кнопки».


[![](../../../opened.gif)![](../../../closed.gif)Для открытия окна](javascript:TextPopup(this))


		- Выполните команду ![](../b_params.png) «Настроить
	 ...
	 Общее Действие Дополнительно
		...
		Задайте общие настройки кнопки:
			...
			- Наименование кнопки.
			 ...
			 «Пользовательская кнопка N», где N - номер кнопки;


			- Изображение для кнопки.
			 Установите пиктограмму кнопки, которая будет отображаться
			 на [панели
			 инструментов](../../organizational_management/Starting.htm#structure_window). Для этого нажмите кнопку «Загрузить
		...
		Для сброса заданного изображения
		 и установки пиктограммы по умолчанию нажмите кнопку ![](../OperationReport/b_delete.png) «Сбросить
		 пиктограмму».
		...
		[![](../../../opened.gif)![](../../../closed.gif)Настройка
		 открытия объекта репозитория](javascript:TextPopup(this))
			...
			В качестве объекта репозитория
			 можно выбрать [форму
			 ввода](dataentryforms.chm::/DataEntryForms_Purpose.htm), регламентный отчёт, [веб-форму](uidevenv.chm::/01_Development_Environment/02_Work_in_Development_Environment/DevEnv_Object/Web_Form.htm)
			 или [справочник
...
Выбрать можно несколько вариантов. Идентификатор
 и ключ будут указаны в скобках;
			...
			Примечание.
			 ...
			 в иерархию объектов, из которых состоит отчёт. Для просмотра
			 иерархии объектов отчёта используйте вкладку «[Состоит
			 ...
			 объекта» в навигаторе объектов.
				...
				- Установите флажок при необходимости:
					...
					- Открывать в
					 ...
					 установке флажка блокируется работа с родительским
					 отчётом во время открытия объекта;
					...
					- Способ задания.
					 В раскрывающемся списке выберите:
						...
						- Параметр
						 ...
						 передается значение параметра отчёта или формы
						 ввода;
				...
				- Значение.
				 ...
				 значения:
					...
					- если был выбран способ передачи значения «Параметр объекта»,
					 ...
					 отображаются доступные параметры отчёта. Список параметров
					 отчёта отфильтрован по типу параметра объекта. Выбрать
					...
					- если был выбран способ передачи значения «Атрибут измерения»,
					 ...
					 всех табличных областей в отчёте. Выбрать можно только
					 один атрибут;
...
### Особенности работы с веб-формами
			...
			Если по пользовательской кнопке открывается веб-форма, то
			 ...
			 регламентный отчёт. Активный отчёт возвращает статическое
			 свойство [PrxReport.ActiveReport](KeReport.chm::/Interface/IPrxReportClass/IPrxReportClass.ActiveReport.htm),
...
Выбрать можно несколько вариантов. Идентификатор
 и ключ будут указаны в скобках;
			...
			Примечание.
			 ...
			 включается в иерархию объектов, из которых состоит отчёт.
			 Для просмотра иерархии объектов отчёта используйте вкладку
			 «[Состоит
			 из](uinav.chm::/03_objects/uinav_obj_basicprop.htm)» окна «Свойства
			 объекта» в навигаторе объектов.
...
Параметры:
	...
	- пользовательские параметры.
	 Значения пользовательских параметров в соответствии с сигнатурой вызываемой
	...
	- args. Аргументы функции,
	 генерируемые платформой. Необязательный параметр, позволяет получить
	 доступ к объекту, из которого была выполнена js-функция.
...
Для получения подробной информации о сигнатуре
 и подключении js-функции к регламентному отчёту обратитесь к разделу «[Подключение пользовательской
...
Параметр args
 ...
 представлено ниже.
	...
	- Platform.Bi
	 - объект, содержащий методы для взаимодействия с BI-сервером посредством
	 Json-запросов. Список методов и их сигнатура:


		- execMethod('ID репозитория',
		 'ID сборки', 'Метод', ['Параметр 1','Параметр 2',…]) - для выполнения
		 запросов с помощью операции [ForeExec](PPSOMService.chm::/Operations/Repository/ForeExec.htm).


		- sendRequest({параметры}, Json-запрос
		 1, Json-запрос 2,…) - для выполнения пакета Json-запросов. Каждый
		 отдельный запрос осуществляет выполнение одной из доступных [операций](PPSOMService.chm::/Operations/BaseService.htm)
		 BI-сервера. В качестве параметров в текущей реализации может быть
		 указан параметр batched: boolean,
		 указывающий необходимость выполнения указанных запросов в виде
		 одного пакета.


		- putBin('моникёр',
		 FormData) - для загрузки изображения в документ с помощью
		 операции [PutBin](PPSOMService.chm::/Additional/PutBin.htm).


		- getBin('моникёр', признак загрузки,
		 'имя файла') - для получения изображения из документа с помощью
		 операции [GetBin](PPSOMService.chm::/Additional/GetBin.htm).
	 ...
	 под оформление платформы. Список методов и их сигнатура:
		...
		- Platform.Managers -
		 ...
		 Каждый объект имеет два поля:
			...
			- selectors. Содержит
			 селекторы для получения информации об открытом объекте:
		...
		- actions.
		 Содержит метод refresh() для обновления открытого объекта.
...
// Вызов метода в сборке репозитория
		...
		[![](../../../opened.gif)![](../../../closed.gif)Настройка
		 управления активностью кнопки](javascript:TextPopup(this))
			...
			Примечание.
			 ...
			 должна быстро выполняться, иначе регламентный отчёт будет
			 неработоспособным.
...
[Настройки
 панели инструментов](../../organizational_management/Toolbar_settings.htm)
