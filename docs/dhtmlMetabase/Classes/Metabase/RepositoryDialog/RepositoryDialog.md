...
Класс RepositoryDialog реализует
 диалог для выбора репозитория НСИ и места хранения результирующего куба,
 который открывается при нажатии на кнопку «Дополнительно» на [третьей
 странице](../BindingsStep/BindingsStep.htm) [мастера
 импорта данных](../ImportDataWizard/ImportDataWizard.htm).
...
PP.initClass(PP.Mb.Ui.RepositoryDialog, PP.Ui.[Dialog](dhtmlUi.chm::/Classes/Dialog/Dialog.htm), "RepositoryDialog");
...
## Конструктор
		 ...
		 ![](../../../Sub_Image.gif)
		 RepositoryDialog
		 Конструктор RepositoryDialog создает экземпляр компонента RepositoryDialog.
