...
Класс ConflictDialog реализует
 диалог конфликтов импорта данных, который открывается при конфликтах импорта
 [мастера импорта
...
PP.initClass(PP.Mb.Ui.ConflictDialog, PP.Ui.[Dialog](dhtmlUi.chm::/Classes/Dialog/Dialog.htm),
 "PP.Mb.Ui.ConflictDialog");
...
## Конструктор
		 ...
		 ![](../../../../Sub_Image.gif)
		 ConflictDialog
		 Конструктор ConflictDialog
		 создает экземпляр компонента ConflictDialog.
