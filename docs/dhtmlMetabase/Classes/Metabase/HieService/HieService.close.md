...
Для выполнения примера необходимо создать сервис для работы с временными
 рядами (см. «[HieService.getChildEls](HieService.getChildEls.htm)»)
...
После выполнения примера объект иерархии будет закрыт, а в консоль браузера
...
PP.Mb.HieService {_Metabase: PP.Mb.Metabase, _UseMbQueueRqts:
 true, _Events: Object, MetadataChanged: PP.Delegate, MetadataChangeStart:
 PP.Delegate…}
