...
Класс MetabaseTreeList реализует компонент [MetabaseTreeList](../../../Components/Metabase/MetabaseTreeList/MetabaseTreeList.htm), который представляет собой иерархическое дерево для отображения списка объектов репозитория.
...
PP.initClass(PP.Mb.Ui.MetabaseTreeList, PP.Ui.[ProxyControl](dhtmlUi.chm::/Classes/ProxyControl/ProxyControl.htm), "MetabaseTreeList");
