...
Свойство Dim определяет измерение
 по ключу атрибута.
...
Для выполнения примера необходимо создать сервис для работы с временными
 рядами с наименованием «hieSrv» (см. «[HieService.getChildEls](../HieService/HieService.getChildEls.htm)»).
 В обработчике события открытия документа добавить следующий код:
...
После выполнения примера в консоль браузера будет выведен json-объект
 измерения по ключу первого атрибута и соответствующее сообщение.
