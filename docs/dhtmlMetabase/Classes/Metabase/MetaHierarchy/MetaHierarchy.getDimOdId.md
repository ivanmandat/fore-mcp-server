...
Для выполнения примера предполагается наличие измерения с ключом 0.
 Необходимо создать сервис для работы с временными рядами, получить экземпляр
 рабочей книги из базы данных временных рядов, получить экземпляр иерархии
 метаданных с наименованием «hie» (см. «[MetaHierarchy.ChildEls](MetaHierarchy.ChildEls.htm)»)
