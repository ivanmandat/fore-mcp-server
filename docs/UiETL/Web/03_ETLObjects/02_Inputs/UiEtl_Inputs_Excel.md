# Коннектор к источнику данных «Excel»: Задача ETL, веб-приложение
...
# Excel
	...
	Примечание.
	 Для импорта данных из файлов Microsoft Excel с большим объемом данных
	 может потребоваться большое количество оперативной памяти. Для предотвращения
	 такой ситуации сохраните данные из файла Excel в csv-файл и воспользуйтесь
	 коннектором «[Файл](UiEtl_Inputs_text.htm)».
 ...
 исключительная ситуация.
	...
	- Примечание. При необходимости
	 ...
	 усмотрению пользователя;
		...
		- Лист данных. В раскрывающемся
		 списке «Лист данных» выберите
		 лист, с которого будут импортированы данные;
		...
		- Параметры импорта.
		 Для настройки импорта укажите параметры:
			...
			- Пропустить первые N
			 строк. По умолчанию указан «0», данные будут импортированы
			 с начала листа. При необходимости, укажите количество строк,
			 которые необходимо пропустить при импорте;


			- Строка с заголовками.
			 По умолчанию установлено значение «0», считается, что заголовки
			 в файле отсутствуют. Наименования заголовков будут сгенерированы
			 автоматически. Для использования другой строки установите
			 соответствующий номер строки, из которой будут получены наименования
			 заголовков;


		- Все данные имеют строковый
		 тип. По умолчанию переключатель выключен. Для приведения
		 всех данных к строковому типу и импорта данных в строковые поля
		 включите переключатель;


		- Определить автоматически
		 по первым строкам. По умолчанию переключатель включен.
		 Тип данных будет определяться автоматически по первым 100 строкам.
		 При необходимости, измените количество строк, по которым будет
