# Коннектор к источнику данных «JSON»: Задача ETL, веб-приложение
...
# JSON
	...
	Коннектор «JSON» - объект,
	 ...
	 полученный с какого-либо веб-сайта.
...
## Настройка импорта
	...
	На панели свойств указывается файл с импортируемыми данными и JSONPath-запрос,
	 который используется для извлечения данных.
 ...
 исключительная ситуация.
	...
	- Примечание. При необходимости
	 ...
	 усмотрению пользователя;
		...
		- JsonPath-запрос.
		 В поле «JSONPath-запрос»
		 укажите запрос, в соответствии с которым будут отбираться импортируемые
		 данные;


	Примечание.
	 Более подробную информацию о формировании JSONPath-запросов можно
	 найти в специализированной документации, например, в MSDN.


	- Все данные имеют строковый
	 тип. По умолчанию переключатель выключен. Для приведения всех
	 данных к строковому типу и импорта данных в строковые поля включите
	 переключатель;


	- Определить автоматически по
	 первым строкам. По умолчанию переключатель включен. Тип данных
	 будет определяться автоматически по первым 100 строкам. При необходимости,
	 измените количество строк, по которым будет определяться тип данных;
