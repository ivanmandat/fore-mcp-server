# Приёмник данных «XML»: Задача ETL, настольное приложение
...
# XML
	...
	Коннектор «XML» - объект,
	 предназначенный для экспорта данных в XML-файл (*.xml).
...
## Настройка экспорта
	...
	В поле «Имя корневой вершины»
	 укажите наименование обязательного корневого элемента выходного XML-файла,
	 а в поле «Имя вершины данных»
	 - наименование элемента, который будет содержать данные.


	В группе «Формат файла»
	 содержатся переключатели, определяющие формат выходного файла:


		- Поля в вершинах.
		 Входные поля будут дочерними по отношению к элементу, указанному
		 в поле «Имя вершины данных»;


		- Поля в атрибутах.
		 Входные поля будут атрибутами элемента, указанного в поле «Имя вершины данных».
