# Приёмник данных «Access»: Задача ETL, настольное приложение
...
# Access
	...
	Коннектор «Access» - объект,
	 предназначенный для экспорта данных в файлы формата Microsoft Access
	 (*.mdb).
...
## Настройка экспорта
	...
	На странице «Настройка экспорта»
	 указывается файл базы для экспорта данных.


	![](../UiEtl_Outputs_Access.gif)


	Укажите место расположения файла базы данных Microsoft Access, в
	 который будут выгружены данные.


	Данные будут выгружены в таблицу, указанную в раскрывающемся списке
	 «Таблица». Список содержит
	 все таблицы выбранной БД.
