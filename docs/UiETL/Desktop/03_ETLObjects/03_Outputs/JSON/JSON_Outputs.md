# Приёмник данных «JSON»: Задача ETL, настольное приложение
...
# JSON
	...
	Коннектор «JSON» - объект, предназначенный
    для экспорта данных в файлы в виде JSON-структур.
...
## Страница «Настройка экспорта»
	...
	Задайте параметры экспорта данных в JSON-файл:


		- Текстовый файл. Укажите
         место расположения файла, который является приёмником данных;


		- Имя корневого элемента.
		 Задайте имя корневого элемента, относительно которого данные будут
		 экспортированы в виде дочерних элементов. По умолчанию задается наименование
         «Data», при этом во время экспорта будет получена JSON-структура следующего
         вида: {"Data":[{…},{…},…]};


		- Очищать приёмник перед загрузкой.
         Установка флажка позволяет удалить всю информация из приёмника перед
         выполнением загрузки данных.
