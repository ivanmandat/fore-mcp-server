# Источник данных «Dbase»: Задача ETL, настольное приложение
...
# Dbase
	...
	Коннектор «Dbase» - объект,
	 предназначенный для извлечения данных из файлов формата DBase III,
	 DBase IV, DBase 5 или VFPOLEDB.1 (*.dbf).


	Примечание.
	 Драйвер VFPOLEDB.1 может использоваться только в 32-битной версии
...
## Настройка импорта
	...
	![](UiEtl_Inputs_Dbase.gif)


	Укажите путь к каталогу с файлами Dbase и драйвер для соответствующей
	 версии Dbase.
	...
	Возможны ситуации неудачного импорта с помощью всех доступных драйверов.
     ...
     источник «[Visual
	 FoxPro](UiEtl_Inputs_VisualFoxPro.htm)».


	В поле «Запрос» введите
	 текст запроса, по которому будет осуществляться выборка данных.
