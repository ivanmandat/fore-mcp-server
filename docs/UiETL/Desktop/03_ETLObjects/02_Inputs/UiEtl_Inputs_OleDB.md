# Источник данных «OLE DB»: Задача ETL, настольное приложение
...
# OLE DB
	...
	Коннектор «OLE DB» - объект,
	 предназначенный для извлечения данных БД с использованием драйверов OLE DB,
	 установленных в операционной системе**.


	**OLE DB** (англ. *Object Linking and Embedding, Database*) -
	 набор интерфейсов, основанных на объектной модели компонентов, которые
	 позволяют обращаться к данным, хранимым в разных источниках информации
	 или хранилищах данных с помощью унифицированного доступа.
...
## Настройка импорта
	...
	На странице «Настройка импорта»
	 указываются параметры подключения к источнику и запрос, в соответствии
	 с которым будут извлекаться данные из источника:


	![](UiEtl_Inputs_OleDB.gif)
	...
	В поле «Запрос» введите текст
     запроса, по которому будет осуществляться выборка данных.
