# Источник данных «JSON»: Задача ETL, настольное приложение
...
# JSON
	...
	Коннектор «JSON» - объект,
	 предназначенный для извлечения данных из файлов в формате JSON (*.json).
	 В качестве источника может выступать как локальный файл, так и файл,
	 который будет получен с какого-либо веб-сайта.
...
## Настройка импорта
	...
	На странице «Настройка импорта»
	 указывается файл с импортируемыми данными и JSONPath-запрос, который
	 используется для извлечения данных:


	![](JSON/JSON_Import_Settings.png)


	В поле «Исходный файл» укажите
	 место расположения JSON-файла, который будет являться источником данных.
	 Для выбора файла нажмите кнопку «Обзор».
	 В поле «JSONPath-запрос» укажите
	 запрос, в соответствии с которым будут отбираться импортируемые данные.
	...
	Примечание.
	 ...
	 подробную информацию о формировании JSONPath-запросов можно найти
	 в специализированной документации, например, в MSDN.


	Если установить флажок «Запрашивать
	 файл при выполнении задачи», то при выполнении задачи ETL будет
	 открыт диалог для выбора JSON-файла. По умолчанию флажок снят, при
	 этом работа осуществляется с тем файлом, который указан в поле «Исходный файл».
	...
	Кнопка «Просмотр данных»
	 позволяет просмотреть первые сто записей, отобранных по запросу.
