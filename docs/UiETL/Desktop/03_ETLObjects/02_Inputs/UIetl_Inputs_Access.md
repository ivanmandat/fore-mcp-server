# Источник данных «Access»: Задача ETL, настольное приложение
...
# Access
	...
	Коннектор «Access» - объект,
	 предназначенный для извлечения данных из файлов формата Microsoft
	 Access (*.mdb).
...
## Настройка импорта
	...
	![](UIetl_Inputs_Access.gif)


	В поле «Файл базы данных Access»
	 укажите место расположения файла базы данных Access, который будет
	 являться источником данных. Для выбора файла нажмите кнопку «Обзор».


	В поле ввода «Запрос» введите
	 текст запроса, по которому будет осуществляться выборка данных. Подробную
	 информацию о синтаксисе можно найти в документации Microsoft Access.


	Если установить флажок «Запрашивать
	 файл при выполнении задачи», то при выполнении задачи ETL будет
	 открыт диалог для выбора файла базы данных Microsoft Access. По умолчанию
	 флажок снят, при этом работа осуществляется с тем файлом, который
	 указан в поле «Файл базы данных Access».
