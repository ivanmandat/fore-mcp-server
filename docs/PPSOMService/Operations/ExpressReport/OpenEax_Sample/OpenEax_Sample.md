# Открытие экспресс-отчета: Операция OpenEax
...
Ниже приведен пример использования операции [OpenEax](../OpenEax.htm)
 для открытия экспресс-отчета. В запросе передается моникёр объекта, являющегося
 экспресс-отчетом. В ответе приходит моникёр открытого экземпляра экспресс-отчета.
