...
Для подключения к многомерным базам данных на базе SAP HANA предварительно
 необходимо установить клиентскую часть, содержащую необходимый OLE DB
 драйвер. Разрядность должна совпадать с разрядностью используемой версии
...
Для создания подключения к SAP HANA выполните следующие действия:
...
Нажмите кнопку «Далее»
 для перехода к странице мастера «Параметры».
	...
	- Выберите драйвер «SAP HANA»,
	 в поле «Сервер» укажите IP-адрес
	 или наименование сервера SAP HANA. В поле «Инстанс»
	 укажите номер экземпляра базы данных. Номер экземпляра можно получить
	 у администратора баз данных SAP HANA. Для подключения определите способ
	 аутентификации пользователя, при необходимости установите флажок «Автоматическое подключение» и укажите
	 имя пользователя и пароль. Если флажок «Автоматическое
	 ...
	 соединения с репозиторием. Если установить флажок «Автоматическое
...
![](ADOMD%20master%20Page%203_3.png)
...
[Подключение
 к многомерным базам данных ADOMD](UiDb_relational_ADOMD.htm)
