...
Свойство Selection определяет
 отметку элементов по которым будет производиться расчет.
...
Актуально, если свойству [Type](IMDCalculationSliceFilter.Type.htm)
 установлено значение MDCalculationSliceFilterType.Selection.
...
Для выполнения примера предполагается наличие в репозитории многомерного
 расчета на сервере БД с идентификатором MDCalc_1. Приёмник данных построен
...
После выполнения примера будет осуществлен многомерный расчет на сервере
