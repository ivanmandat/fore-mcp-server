...
Свойство TextFontStyleUnderline
 определяет использование подчеркнутого начертания шрифта. Если значение
 True, то используется подчеркнутое
 начертание, при значении False
 - нет.
...
После выполнения примера объект будет содержать текст заданного цвета,
 размера и будет подчеркнутого начертания.
