...
Класс EaxModuleObject реализует
 объект модуля с загруженным в него экспресс-отчётом.


## Синтаксис


PP.initClass(PP.Application.EaxModuleObject, [PP.Application.ModuleObject](../ModuleObject/ModuleObject.htm),
 "PP.Application.EaxModuleObject");
...
## Конструктор
		 ...
		 ![](../../../sub_image.gif)
		 [EaxModuleObject](Constructor_EaxModuleObject.htm)
		 Конструктор EaxModuleObject
		 создаёт экземпляр класса EaxModuleObject.
