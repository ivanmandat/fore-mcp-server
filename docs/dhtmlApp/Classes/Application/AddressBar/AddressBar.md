...
Класс AddressBar реализует
 адресную строку приложения.


## Комментарии


Адресная строка может выглядеть следующим образом:
...
PP.initClass(PP.Application.AddressBar, [PP.Ui.Container](dhtmlUi.chm::/Classes/Container/Container.htm),
 "PP.Application.AddressBar");
...
## Конструктор
			 ...
			 ![](../../../sub_image.gif)
			 [AddressBar](Constructor_AddressBar.htm)
			 Конструктор AddressBar
			 создаёт экземпляр класса AddressBar.
