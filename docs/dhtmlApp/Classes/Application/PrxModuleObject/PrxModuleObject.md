...
Класс PrxModuleObject реализует
 объект модуля с загруженным в него регламентным отчётом.


## Синтаксис


PP.initClass(PP.Application.PrxModuleObject, [PP.Application.ModuleObject](../ModuleObject/ModuleObject.htm),
 "PP.Application.PrxModuleObject");
...
## Конструктор
		 ...
		 ![](../../../sub_image.gif)
		 [PrxModuleObject](Constructor_PrxModuleObject.htm)
		 Конструктор PrxModuleObject
		 создает экземпляр класса PrxModuleObject.
