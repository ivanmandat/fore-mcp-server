...
Свойство Proportional определяет,
 будет ли показатель пропорциональным.
...
Допустимые значения:
	...
	- True. Все секторы кругового
	 показателя будут одинаковыми по размеру (изображение слева);


	- False. Размер секторов
	 будет зависеть от данных показателя (изображение справа).


![](IMapPieVisual_Proportional.gif)
...
После выполнения примера на слой «Regions» будет добавлен круговой показатель,
 для штриховки которого будут использованы красный и синий цвета. Размер
 секторов показателя будет зависеть от данных.
