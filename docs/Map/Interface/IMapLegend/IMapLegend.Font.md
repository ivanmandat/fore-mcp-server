...
Добавьте ссылки на системные сборки: Drawing, ExtCtrls, Forms, Map.
	...
	    F := New GxFont.Create("Courier New", 10, GxFontStyle.Bold);

	    // Зададим параметры шрифта элементов легенды

	    Legend.Font := F;

	    // Зададим параметры цвета шрифта элементов легенды

	    Legend.FontColor := GxColor.FromName("Red");
...
После выполнения примера для картографического показателя будет отображена
 легенда. Стандартное оформление шрифта элементов легенды будет изменено:
