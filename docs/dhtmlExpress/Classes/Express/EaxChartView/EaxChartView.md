...
Класс EaxChartView реализует
 представление [диаграммы
 экспресс-отчета](../../../Components/Express/ChartBox/ChartBox.htm).


## Синтаксис


PP.initClass(PP.Exp.Ui.EaxChartView, PP.Ui.[Control](dhtmlUi.chm::/Classes/control/control.htm),
 "EaxChartView", [PP.Exp.Ui.IEaxChartView, PP.Ui.IDataView]);
...
## Конструктор
		 ...
		 ![](../../../sub_image.gif)
		 [EaxChartView](dhtmlExpress.chm::/Classes/Express/EaxChartView/Constructor_EaxChartView.htm)
		 Конструктор EaxChartView
		 создает экземпляр компонента EaxChartView.
