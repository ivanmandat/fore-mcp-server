# EaxBubbleChart.getAreMetricsOn
...
// Получим компонент bubbleChart
dataView=expressBox.getDataView();
bubbleView = dataView.getBubbleChartView();
// Выведем в консоль информацию о наличии измерения метрик
console.debug(bubbleView.getAreMetricsOn() ? "Визуализатор содержит измерение метрик" : "Измерение метрик отсутствует в визуализаторе" );
