...
Класс ChainDialog реализует
 диалог «Цепочка расчета».


## Пример


Для получения экземпляра диалога выполните следующий код:
...
PP.initClass(PP.Exp.Ui.ChainDialog, PP.Ui.[Dialog](dhtmlUi.chm::/Classes/Dialog/Dialog.htm),
 "ChainDialog");
...
## Конструктор
			 ...
			 ![](../../../sub_image.gif)
			 ChainDialog
			 Конструктор ChainDialog
			 создает экземпляр класса ChainDialog.
...
## Свойства
		 ...
		 ![](../../../Property_Image.gif)
		 [Source](ChainDialog.Source.htm)
		 Свойство Source определяет
		 источник данных диалога.
...
## Методы
		 ...
		 ![](../../../sub_image.gif)
		 [getItems](ChainDialog.getItems.htm)
		 Метод getItems возвращает
		 формулы, отображенные в диалоге цепочки расчета.


		 ![](../../../sub_image.gif)
		 [refresh](ChainDialog.refresh.htm)
		 Метод refresh обновляет
		 диалог цепочки расчета.
...
## События
		 ...
		 ![](../../../Event_Image.gif)
		 [FormulaChanged](ChainDialog.FormulaChanged.htm)
		 Событие FormulaChanged
		 наступает после изменения формулы в диалоге.
