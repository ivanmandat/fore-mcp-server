# Добавление параметров: Алгоритм расчёта показателей, веб-приложение
...
Параметры алгоритма расчёта позволяют динамически управлять [расчётом
 связанных блоков](Manage_settings.htm).


Добавление параметров алгоритма расчёта выполняется на странице «Параметры» в [мастере
 редактирования настроек алгоритма расчёта](Set_up_calculation_parameters.htm). Для задания значений параметров,
 которые будут использоваться при [расчёте
 алгоритма](Perform_calculations.htm), используйте [панель параметров](Beginning_of_work.htm).
 Порядок отображения параметров на данной панели совпадает с порядком следования
 параметров на странице «Параметры»:
...
[![](../../Opened.gif)![](../../Closed.gif)Добавление параметра](javascript:TextPopup(this))
	...
	Для добавления нового параметра нажмите кнопку ![](b_add.png)
	...
	Для добавления существующего параметра из другого блока или
	 параметра из алгоритма расчёта:
		...
		- Вызовите раскрывающееся меню кнопки ![](b_add.png)
...
[![](../../Opened.gif)![](../../Closed.gif)Изменение
 порядка следования параметров](javascript:TextPopup(this))
	...
	Порядок следования параметров влияет на порядок их отображения на
	 странице «[Управление
	 параметрами](Manage_settings.htm)» в [мастере
	 редактирования параметров алгоритма расчёта](Set_up_calculation_parameters.htm).
...
[![](../../Opened.gif)![](../../Closed.gif)Выбор отображаемых
 столбцов](javascript:TextPopup(this))
	...
	Для выбора отображаемых столбцов в таблице параметров установите
	 флажки напротив столбцов в раскрывающемся списке кнопки ![](../Calculation_block/b_params.png) «Выбрать
...
[![](../../Opened.gif)![](../../Closed.gif)Удаление
 параметра](javascript:TextPopup(this))
	...
	Для удаления параметра выделите строку с параметром и нажмите кнопку
	 ![](../Calculation_block/delete.png) «Удалить».
...
Для быстрого выбора объекта в поле для поиска
 ...
 нажмите кнопку ![](../Calculation_block/b_params.png) «Отображение
...
Для отмены выбора куба, заданного в качестве
 ...
 нажмите кнопку ![](b_close.png) «Очистить»,
...
Для быстрого выбора объекта в поле для поиска
 ...
 кнопку ![](../Calculation_block/b_params.png) «Отображение
 объекта» и выберите в раскрывающемся меню вариант отображения:
...
Примечание.
 При изменении типа параметра значение по умолчанию сбрасывается.
	...
	- Отображать на панели параметров.
	 ...
	 параметров](Beginning_of_work.htm). При [расчёте алгоритма](Perform_calculations.htm)
	 будет использоваться значение параметра, заданное по умолчанию. По
	 умолчанию флажок установлен, значения параметров доступны для изменения
	 на панели параметров перед расчётом алгоритма.
