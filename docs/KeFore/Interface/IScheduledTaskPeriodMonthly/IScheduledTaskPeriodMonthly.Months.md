...
Months(Index: [CalendarMonth](../../Enums/CalendarMonth.htm)):
 Boolean;
...
Index. Месяц, для которого
 необходимо определить признак расчета.


## Описание


Свойство Months определяет,
 будет ли производиться расчет в месяце, индекс которого передается посредством
