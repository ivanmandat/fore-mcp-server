# IForeSerializerLoader.LoadForm

IForeSerializerLoader.LoadForm
-


# IForeSerializerLoader.LoadForm


## Синтаксис


LoadForm: [IForm](KeSom.chm::/Interface/IForm/IForm.htm);


## Описание


Метод LoadForm осуществляет
 загрузку и сохранение формы в репозиторий.


## Комментарии


Метод осуществляет сохранение формы в соответствии с параметрами, указанными
 в свойстве [CreateInfo](IForeSerializerLoader.CreateInfo.htm).
 Если в репозитории существует объект, имеющий такой же идентификатор,
 как и у загружаемой формы, то будет сгенерирована исключительная ситуация.


## Пример


Для выполнения примера предполагается наличие файла «c:\Form_1.ppform»,
 содержащего сохраненную форму платформы.


			Sub UserProc;

Var

    MB: IMetabase;

    Fs: IForeSerializer;

    Loader: IForeSerializerLoader;

    Id: String;

Begin

    MB := MetabaseClass.Active;

    Fs := New ForeSerializer.Create;

    Loader := Fs.CreateLoaderFromFile("c:\Form_1.ppform", MB);

    Id := Loader.CreateInfo.Id;

    If MB.ItemById(Id) <> Null Then

        Loader.CreateInfo.Id := MB.GenerateId(Id);

        Loader.CreateInfo.Name := Loader.CreateInfo.Name + "_Copy";

    End If;

    Loader.CreateInfo.Parent := MB.Root;

    Loader.LoadForm;

End Sub UserProc;


При выполнении примера из указанного файла будет загружена форма. Форма
 будет сохранена в корневом каталоге репозитория. Если в репозитории существует
 объект, имеющий такой же идентификатор как и загружаемая форма, то перед
 загрузкой идентификатор формы будет изменен.


См. также:


[IForeSerializerLoader](IForeSerializerLoader.htm)


		Справочная
		 система на версию 10.9
		 от 18/08/2025,
		 © ООО «ФОРСАЙТ»,
