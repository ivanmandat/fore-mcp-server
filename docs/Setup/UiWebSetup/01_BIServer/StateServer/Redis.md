...
Для настройки сервера состояний на базе [Redis](https://github.com/redis/redis):
	...
	- Создайте подраздел [StateServer](../../03_Setup_Web/BI_Server_Registry_Key.htm#stateserver)
	 ...
	 Заполните подраздел параметрами подключения к серверу состояний:
		...
		- Port. Номер порта,
		 ...
		 принимает значение «-1», то используется порт по умолчанию: 6379;


		- ExpireTime. Время
		 хранения записей на сервере состояний в секундах. Если параметр
		 не задан или равен 0, то записи хранятся бессрочно.
