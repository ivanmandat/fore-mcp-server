...
Компонент DropPanel представляет собой раскрывающуюся панель с произвольным содержимым.


## Комментарии


Компонент реализован классом [DropPanel](../../Classes/DropPanel/DropPanel.htm).
...
Размеры компонента можно изменять путем перетаскивания его границ. Содержимое компонента может быть произвольным. Если содержимое компонента не вмещается в установленные границы, появляются полосы прокрутки.


[Пример создания компонента DropPanel](Example_DropPanel.htm)


[Созданиe раскрывающейся панели с полосами прокрутки](../ScrollBox/DropPanel_and_ScrollBox.htm)
...
## Конструктор
		 ...
		 ![](../../Sub_Image.gif)
		 [DropPanel](dhtmlUi.chm::/Classes\DropPanel\Constructor_DropPanel.htm)
		 Конструктор DropPanel
		 создает экземпляр компонента DropPanel.
