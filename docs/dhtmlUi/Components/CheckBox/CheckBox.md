...
Компонент CheckBox представляет
 собой флажок, используемый для индикации состояния или для включения и
 выключения каких-либо опций. Внешний вид компонента меняется при щелчке
 пользователя по флажку. Всего доступно три состояния компонента:
...
По умолчанию доступны отмеченное и неотмеченное состояния. Промежуточное
 состояние доступно при включении режима трех состояний, то есть при установке
 для свойства [ThreeState](../../Classes/CheckBox/CheckBox.ThreeState.htm)
 значения true.
...
Компонент реализован классом [CheckBox](../../Classes/CheckBox/CheckBox.htm).


## Пример


Три состояния компонента CheckBox:


![](CheckBox2.gif)


[Пример создания компонента CheckBox](Example_CheckBox.htm)
...
## Конструктор
		 ...
		 ![](../../Sub_Image.gif)
		 [CheckBox](dhtmlUi.chm::/classes/checkbox/constructor_checkbox.htm)
		 Конструктор CheckBox
		 создает экземпляр компонента CheckBox.
