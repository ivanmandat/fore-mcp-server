...
Компонент ToolBar представляет собой панель инструментов. На панели инструментов размещаются кнопки - экземпляры класса [ToolBarButton](../../Classes/ToolBarButton/ToolBarButton.htm) - и разделители - экземпляры класса [ToolBarSeparator](../../Classes/ToolBarSeparator/ToolBarSeparator.htm).


## Комментарии


Компонент реализован классом [ToolBar](../../Classes/ToolBar/ToolBar.htm).
...
Данная панель инструментов включает в себя одну [группу переключателей](../../Classes/ToolBarButton/ToolBarButton.GroupName.htm), одну [группу флажков](../../Classes/ToolBarButton/ToolBarButton.IsToggle.htm) и [кнопку с раскрывающимся меню](../../Classes/ToolBarButton/ToolBarButton.Menu.htm). Между группой кнопок-флажков и кнопкой с раскрывающимся меню размещен разделитель в виде линии.


[Пример создания компонента ToolBar](Example_ToolBar.htm)
...
## Конструктор
		 ...
		 ![](../../Sub_Image.gif)
		 [ToolBar](dhtmlUi.chm::/classes/toolbar/constructor_toolbar.htm)
		 Конструктор ToolBar
		 создает экземпляр компонента ToolBar.
...
## Свойства
		 ...
		 ![](../../Property_Image.gif)
		 [AutoFit](dhtmlUi.chm::/Classes/ToolBar/ToolBar.AutoFit.htm)
		 Свойство IsSplitGroups
		 определяет, будет ли добавлен отступ между элементами группы панели
		 инструментов.


		 ![](../../Property_Image.gif)
		 [ImageList](dhtmlUi.chm::/classes/toolbar/toolbar.imagelist.htm)
		 Свойство ImageList
		 возвращает экземпляр невизуального компонента [ImageList](dhtmlUi.chm::/components/imagelist/imagelist.htm).


		 ![](../../Property_Image.gif)
		 [IsSplitGroups](dhtmlUi.chm::/Classes/ToolBar/ToolBar.IsSplitGroups.htm)
		 Свойство IsSplitGroups
		 определяет, будут ли разделены элементы внутри группы панели инструментов.


		 ![](../../Property_Image.gif)
		 [Items](dhtmlUi.chm::/Classes/ToolBar/ToolBar.Items.htm)
		 Свойство Items содержит
		 массив элементов панели инструментов.


		 ![](../../Property_Image.gif)
		 [Orientation](dhtmlUi.chm::/Classes/ToolBar/ToolBar.Orientation.htm)
		 Свойство Orientation
		 устанавливает ориентацию для компонента ToolBar.


		 ![](../../Property_Image.gif)
		 [ShowOverflowButton](dhtmlUi.chm::/Classes/ToolBar/ToolBar.ShowOverflowButton.htm)
		 Свойство ShowOverflowButton
		 определяет наличие кнопки меню со скрытыми элементам.
...
## Методы
		 ...
		 ![](../../Sub_Image.gif)
		 [addItem](dhtmlUi.chm::/classes/toolbar/toolbar.additem.htm)
		 Метод addItem добавляет
		 в панель инструментов новый дочерний элемент.


		 ![](../../Sub_Image.gif)
		 [beginUpdate](dhtmlUi.chm::/Classes/ToolBar/ToolBar.beginUpdate.htm)
		 Метод beginUpdate отключает
		 перерисовку панели инструментов до тех пор, пока не будет вызван
		 метод [endUpdate](dhtmlUi.chm::/Classes/ToolBar/ToolBar.endUpdate.htm).


		 ![](../../Sub_Image.gif)
		 [endUpdate](dhtmlUi.chm::/Classes/ToolBar/ToolBar.endUpdate.htm)
		 Метод endUpdate включает
		 перерисовку панели инструментов и обновляет её.


		 ![](../../Sub_Image.gif)
		 [getImageList](dhtmlUi.chm::/classes/toolbar/toolbar.getimagelist.htm)
		 Метод getImageList возвращает
		 значение свойства [ImageList](dhtmlUi.chm::/classes/toolbar/toolbar.imagelist.htm).


		 ![](../../Sub_Image.gif)
		 [getItem](dhtmlUi.chm::/classes/toolbar/toolbar.getitem.htm)
		 Метод getItem возвращает
		 дочерний элемент панели инструментов по указанному индексу.


		 ![](../../Sub_Image.gif)
		 [getItemById](dhtmlUi.chm::/classes/toolbar/toolbar.getitembyid.htm)
		 Метод getItemById возвращает
		 дочерний элемент панели инструментов по указанному идентификатору.


		 ![](../../Sub_Image.gif)
		 [getItemsByGroupName](dhtmlUi.chm::/classes/toolbar/toolbar.getitemsbygroupname.htm)
		 Метод getItemsByGroupName
		 возвращает список элементов управления панели по указанному имени
		 группы.


		 ![](../../Sub_Image.gif)
		 [insertItem](dhtmlUi.chm::/Classes/ToolBar/ToolBar.insertItem.htm)
		 Метод insertItem вставляет
		 новый элемент панели инструментов в указанную позицию.


		 ![](../../Sub_Image.gif)
		 [refreshGroup](dhtmlUi.chm::/Classes/ToolBar/ToolBar.refreshGroup.htm)
		 Метод refreshGroup
		 обновляет группу по имени.


		 ![](../../Sub_Image.gif)
		 [refreshGroups](dhtmlUi.chm::/Classes/ToolBar/ToolBar.refreshGroups.htm)
		 Метод refreshGroups
		 обновляет все группы панели инструментов.


		 ![](../../Sub_Image.gif)
		 [refreshItemsVisibility](dhtmlUi.chm::/Classes/ToolBar/ToolBar.refreshItemsVisibility.htm)
		 Метод refreshItemsVisibility
		 обновляет видимость элементов панели инструментов.


		 ![](../../Sub_Image.gif)
		 [removeItem](dhtmlUi.chm::/classes/toolbar/toolbar.removeitem.htm)
		 Метод removeItem удаляет
		 дочерний элемент панели инструментов.


		 ![](../../Sub_Image.gif)
		 [removeItemById](dhtmlUi.chm::/classes/toolbar/toolbar.removeitembyid.htm)
		 Метод removeItemById
		 удаляет дочерний элемент по указанному идентификатору.


		 ![](../../Sub_Image.gif)
		 [removeItemByIndex](dhtmlUi.chm::/classes/toolbar/toolbar.removeitembyindex.htm)
		 Метод removeItemByIndex
		 удаляет дочерний элемент панели инструментов по индексу.


		 ![](../../Sub_Image.gif)
		 [removeItems](dhtmlUi.chm::/Classes/ToolBar/ToolBar.removeItems.htm)
		 Метод removeItems удаляет
		 все дочерние элементы панели инструментов.
