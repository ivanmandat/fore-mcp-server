...
Класс Label реализует компонент
 [Label](../../Components/Label/Label.htm), представляющий собой
 подпись.
...
PP.initClass(PP.Ui.Label, PP.Ui.[Control](../Control/Control.htm),
 "Label");
...
## Конструктор
			 ...
			 ![](../../Sub_Image.gif)
			 [Label](Constructor_Label.htm)
			 Конструктор Label
			 создает экземпляр компонента Label.
