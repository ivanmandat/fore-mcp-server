...
Класс AutoRefreshMenu реализует
 меню автообновления.
...
PP.initClass(PP.Ui.AutoRefreshMenu, [PP.Ui.Menu](../Menu/Menu.htm),
 "PP.Ui.AutoRefreshMenu");
...
## Конструктор
			 ...
			 ![](../../Sub_Image.gif)
			 AutoRefreshMenu
			 Конструктор AutoRefreshMenu
			 создает экземпляр класса AutoRefreshMenu.
...
## Свойства
		 ...
		 ![](../../Property_Image.gif)
		 [Period](AutoRefreshMenu.Period.htm)
		 Свойство Period определяет
		 параметры периодического обновления.


		 ![](../../Property_Image.gif)
		 [ViewerUpdateSettings](AutoRefreshMenu.ViewerUpdateSettings.htm)
...
## Методы
		 ...
		 ![](../../Sub_Image.gif)
		 [getMarkChangedItem](AutoRefreshMenu.getMarkChangedItem.htm)
		 Метод getMarkChangedItem
		 возвращает пункт меню, отвечающий за настройку автообновления
		 при смене отметки.


		 ![](../../Sub_Image.gif)
		 [getPeriodDialog](AutoRefreshMenu.getPeriodDialog.htm)
		 Метод getPeriodDialog
		 возвращает окно настройки периода обновления.


		 ![](../../Sub_Image.gif)
		 [getPeriodicalDisableItem](AutoRefreshMenu.getPeriodicalDisableItem.htm)
		 Метод getPeriodicalDisableItem
		 возвращает пункт меню, отвечающий за отключение периодического
		 обновления.


		 ![](../../Sub_Image.gif)
		 [getPeriodicalIntervalItem](AutoRefreshMenu.getPeriodicalIntervalItem.htm)
		 Метод getPeriodicalIntervalItem
		 возвращает пункт меню, отвечающий за включение периодического
		 обновления.


		 ![](../../Sub_Image.gif)
		 [setMarkChangedRefresh](AutoRefreshMenu.setMarkChangedRefresh.htm)
		 Метод setMarkChangedRefresh
		 включает/отключает автообновление при смене отметки.


		 ![](../../Sub_Image.gif)
		 [setPeriodicalRefresh](AutoRefreshMenu.setPeriodicalRefresh.htm)
		 Метод setPeriodicalRefresh
		 включает/отключает периодическое обновление.


		 ![](../../Sub_Image.gif)
		 [setRefresh](AutoRefreshMenu.setRefresh.htm)
		 Метод setRefresh включает/отключает
		 автообновление при смене отметки и периодическое обновление.


		 ![](../../Sub_Image.gif)
		 [startPeriodicalTimer](AutoRefreshMenu.startPeriodicalTimer.htm)
		 Метод startPeriodicalTimer
		 запускает таймер периодического обновления.


		 ![](../../Sub_Image.gif)
		 [stopPeriodicalTimer](AutoRefreshMenu.stopPeriodicalTimer.htm)
		 Метод stopPeriodicalTimer
		 останавливает таймер периодического обновления.
...
## События
		 ...
		 ![](../../Event_Image.gif)
		 [RefreshStateChanged](AutoRefreshMenu.RefreshStateChanged.htm)
		 Событие RefreshStateChanged
		 генерируется при изменении состояния автообновления.


		 ![](../../Event_Image.gif)
		 [RefreshTimer](AutoRefreshMenu.RefreshTimer.htm)
		 Событие RefreshTimer
		 генерируется при срабатывании таймера периодического обновления.
...
## Событие, унаследованное от класса [Menu](../Menu/Menu.htm)
