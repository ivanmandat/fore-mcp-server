...
Класс ScrollBox реализует компонент [ScrollBox](../../Components/ScrollBox/ScrollBox.htm), представляющий собой горизонтальную и вертикальную полосы прокрутки, предназначенные для просмотра содержимого, размеры которого превышают размеры контейнера.


## Синтаксис


PP.initClass(PP.Ui.ScrollBox, PP.Ui.[Control](../Control/Control.htm), "ScrollBox");
