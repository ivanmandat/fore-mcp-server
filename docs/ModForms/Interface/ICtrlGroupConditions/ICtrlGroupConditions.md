# ICtrlGroupConditions

ICtrlGroupConditions
-


# ICtrlGroupConditions


## Иерархия наследования


           [IComponent](../IComponent/IComponent.htm)


           [IControl](../IControl/IControl.htm)


           [IListView](../IListView/IListView.htm)


           ICtrlGroupConditions


## Описание


Интерфейс ICtrlGroupConditions
 используется для работы с компонентом среды разработки - [CtrlGroupConditions](UiDevEnv.chm::/02_Components_constructor_forms/02_Additional_components/CtrlGroupConditions.htm).


## Свойства


		 Имя свойства
		 Краткое описание


		 ![](../../Property_Image.gif)
		 [Groups](ICtrlGroupConditions.Groups.htm)
		 Свойство Groups определяет
		 коллекцию групп элементов.


		 ![](../../Property_Image.gif)
		 [SelectedRows](ICtrlGroupConditions.SelectedRows.htm)
		 Свойство SelectedRows
		 определяет массив отмеченных элементов.


## Свойства, унаследованные от [IListView](../IListView/IListView.htm)


		 Имя свойства
		 Краткое описание


		 ![](../../Property_Image.gif)
		 [Arrangement](../IListView/IListView.Arrangement.htm)
		 Свойство Arrangement
		 определяет способ выравнивания элементов в компоненте.


		 ![](../../Property_Image.gif)
		 [AutoArrange](../IListView/IListView.AutoArrange.htm)
		 Свойство AutoArrange
		 определяет, будут ли элементы автоматически располагаться правильными
		 рядами при изменении их количества.


		 ![](../../Property_Image.gif)
		 [BorderStyle](../IListView/IListView.BorderStyle.htm)
		 Свойство BorderStyle
		 определяет стиль рамки, обрамляющей компонент.


		 ![](../../Property_Image.gif)
		 [Checkboxes](../IListView/IListView.Checkboxes.htm)
		 Свойство Checkboxes
		 определяет признак наличия флагов у элементов компонента.


		 ![](../../Property_Image.gif)
		 [ColumnClick](../IListView/IListView.ColumnClick.htm)
		 Свойство ColumnClick
		 определяет, будет ли обрабатываться щелчок по заголовкам столбцов.


		 ![](../../Property_Image.gif)
		 [Columns](../IListView/IListView.Columns.htm)
		 Свойство Columns возвращает
		 коллекцию столбцов компонента.


		 ![](../../Property_Image.gif)
		 [FocusedItem](../IListView/IListView.FocusedItem.htm)
		 Свойство FocusedItem
		 определяет элемент компонента, содержащий фокус.


		 ![](../../Property_Image.gif)
		 [GridLines](../IListView/IListView.GridLines.htm)
		 Свойство GridLines
		 определяет, будут ли отображаться линии сетки.


		 ![](../../Property_Image.gif)
		 [HeaderMenu](../IListView/IListView.HeaderMenu.htm)
		 Свойство HeaderMenu
		 определяет контекстное меню, вызываемое по щелчку дополнительной
		 кнопки мыши на заголовке колонки компонента.


		 ![](../../Property_Image.gif)
		 [HideSelection](../IListView/IListView.HideSelection.htm)
		 Свойство HideSelection
		 определяет признак отображения отметки в компоненте при передаче
		 фокуса другому компоненту.


		 ![](../../Property_Image.gif)
		 [HighlightTrack](../IListView/IListView.HighlightTrack.htm)
		 Свойство HighlightTrack
		 определяет, подсвечивать ли элемент при наведении указателя мыши.


		 ![](../../Property_Image.gif)
		 [HotTrackStyles](../IListView/IListView.HotTrackStyles.htm)
		 Свойство HotTrackStyles
		 определяет стиль подсветки элементов, над которыми проходит указатель
		 мыши.


		 ![](../../Property_Image.gif)
		 [HoverTime](../IListView/IListView.HoverTime.htm)
		 Свойство HoverTime
		 определяет промежуток времени, по истечении которого элемент,
		 над которым проходит курсор мыши, будет подсвечиваться.


		 ![](../../Property_Image.gif)
		 [ItemHeight](../IListView/IListView.ItemHeight.htm)
		 Свойство ItemHeight
		 определяет высоту элементов компонента.


		 ![](../../Property_Image.gif)
		 [Items](../IListView/IListView.Items.htm)
		 Свойство Items возвращает
		 коллекцию элементов компонента.


		 ![](../../Property_Image.gif)
		 [LargeImages](../IListView/IListView.LargeImages.htm)
		 Свойство LargeImages
		 определяет источник крупных пиктограмм.


		 ![](../../Property_Image.gif)
		 [MultiSelect](../IListView/IListView.MultiSelect.htm)
		 Свойство MultiSelect
		 определяет возможность множественной отметки элементов списка.


		 ![](../../Property_Image.gif)
		 [ReadOnly](../IListView/IListView.ReadOnly.htm)
		 Свойство ReadOnly определяет
		 возможность внесения изменений в текст элементов списка.


		 ![](../../Property_Image.gif)
		 [RowSelect](../IListView/IListView.RowSelect.htm)
		 Свойство RowSelect
		 определяет необходимость выделения выбранного элемента подсветкой
		 всей строки.


		 ![](../../Property_Image.gif)
		 [SelectedCount](../IListView/IListView.SelectedCount.htm)
		 Свойство SelectedCount
		 возвращает количество выделенных в компоненте элементов.


		 ![](../../Property_Image.gif)
		 [SelectedItem](../IListView/IListView.SelectedItem.htm)
		 Свойство SelectedItem
		 определяет выделенный элемент компонента.


		 ![](../../Property_Image.gif)
		 [ShowColumnHeaders](../IListView/IListView.ShowColumnHeaders.htm)
		 Свойство ShowColumnHeaders
		 определяет, будут ли отображаться заголовки столбцов.


		 ![](../../Property_Image.gif)
		 [ShowHeaderButton](../IListView/IListView.ShowHeaderButton.htm)
		 Свойство ShowHeaderButton
		 определяет, отображается ли кнопка вызова меню заголовков столбцов.


		 ![](../../Property_Image.gif)
		 [ShowHints](../IListView/IListView.ShowHints.htm)
		 Свойство ShowHints
		 определяет, будет ли появляться всплывающая подсказка при задержке
		 курсора мыши над элементами.


		 ![](../../Property_Image.gif)
		 [SmallImages](../IListView/IListView.SmallImages.htm)
		 Свойство SmallImages
		 определяет источник маленьких пиктограмм.


		 ![](../../Property_Image.gif)
		 [SortColumn](../IListView/IListView.SortColumn.htm)
		 Свойство SortColumn
		 определяет индекс колонки, по которой осуществляется сортировка.


		 ![](../../Property_Image.gif)
		 [SortType](../IListView/IListView.SortType.htm)
		 Свойство SortType определяет
		 тип сортировки для компонента.


		 ![](../../Property_Image.gif)
		 [StateImages](../IListView/IListView.StateImages.htm)
		 Свойство StateImages
		 определяет список изображений, используемых для индикации состояния
		 элементов.


		 ![](../../Property_Image.gif)
		 [Style](../IListView/IListView.Style.htm)
		 Свойство Style определяет
		 стиль отображения элементов компонента.


		 ![](../../Property_Image.gif)
		 [TrackSelect](../IListView/IListView.TrackSelect.htm)
		 Свойство TrackSelect
		 определяет, будет ли отмечаться элемент компонента при удержании
		 над ним курсора мыши.


		 ![](../../Property_Image.gif)
		 [WrapText](../IListView/IListView.WrapText.htm)
		 Свойство WrapText определяет,
		 будет ли возможность переноса текста элемента на несколько строк,
		 если он выходит за границы пиктограммы.


## Свойства, унаследованные от [IControl](../IControl/IControl.htm)


		 Имя свойства
		 Краткое описание


		 ![](../../Property_Image.gif)
		 [Align](../IControl/IControl.Align.htm)
		 Свойство Align определяет
		 выравнивание компонента в рамках содержащего его родительского
		 компонента.


		 ![](../../Property_Image.gif)
		 [AllowDrag](../IControl/IControl.AllowDrag.htm)
		 Свойство AllowDrag
		 определяет возможность взять у компонента перетаскиваемый объект.


		 ![](../../Property_Image.gif)
		 [AllowDrop](../IControl/IControl.AllowDrop.htm)
		 Свойство AllowDrop
		 определяет, будет ли возможность у компонента принять перетаскиваемый
		 объект.


		 ![](../../Property_Image.gif)
		 [Anchors](../IControl/IControl.Anchors.htm)
		 Свойство Anchors возвращает
		 настройки, определяющие в процентном соотношении изменение размеров
		 текущего компонента при изменении размеров родительского компонента.


		 ![](../../Property_Image.gif)
		 [Brush](../IControl/IControl.Brush.htm)
		 Свойство Brush определяет
		 кисть, используемую для заливки области компонента.


		 ![](../../Property_Image.gif)
		 [ClientHeight](../IControl/IControl.ClientHeight.htm)
		 Свойство ClientHeight
		 определяет высоту клиентской области компонента.


		 ![](../../Property_Image.gif)
		 [ClientWidth](../IControl/IControl.ClientWidth.htm)
		 Свойство ClientWidth
		 определяет ширину клиентской области компонента.


		 ![](../../Property_Image.gif)
		 [Color](../IControl/IControl.Color.htm)
		 Свойство Color определяет
		 цвет фона компонента.


		 ![](../../Property_Image.gif)
		 [Cursor](../IControl/IControl.Cursor.htm)
		 Свойство Cursor определяет
		 вид курсора, отображаемого при наведении курсора мыши на компонент,
		 во время выполнения формы.


		 ![](../../Property_Image.gif)
		 [Enabled](../IControl/IControl.Enabled.htm)
		 Свойство Enabled определяет
		 доступность компонента для пользователя.


		 ![](../../Property_Image.gif)
		 [Focused](../IControl/IControl.Focused.htm)
		 Свойство Focused возвращает
		 True, если фокус установлен
		 на данном компоненте.


		 ![](../../Property_Image.gif)
		 [Font](../IControl/IControl.Font.htm)
		 Свойство Font определяет
		 параметры шрифта, используемого при отображении текста в компоненте.


		 ![](../../Property_Image.gif)
		 [Height](../IControl/IControl.Height.htm)
		 Свойство Height определяет
		 высоту компонента в пикселях.


		 ![](../../Property_Image.gif)
		 [HelpContext](../IControl/IControl.HelpContext.htm)
		 Свойство HelpContext
		 определяет уникальный индекс раздела контекстно-зависимой справки
		 для данного компонента.


		 ![](../../Property_Image.gif)
		 [Hint](../IControl/IControl.Hint.htm)
		 Свойство Hint определяет
		 текст подсказки, появляющийся при задержке указателя мыши над
		 компонентом.


		 ![](../../Property_Image.gif)
		 [HintTimeout](../IControl/IControl.HintTimeout.htm)
		 Свойство HintTimeout
		 определяет время отображения всплывающей подсказки.


		 ![](../../Property_Image.gif)
		 [Left](../IControl/IControl.Left.htm)
		 Свойство Left определяет
		 координату левого края компонента в пикселях.


		 ![](../../Property_Image.gif)
		 [Parent](../IControl/IControl.Parent.htm)
		 Свойство Parent определяет
		 родительский компонент.


		 ![](../../Property_Image.gif)
		 [ParentColor](../IControl/IControl.ParentColor.htm)
		 Свойство ParentColor
		 определяет, будет ли для компонента заимствован цвет родительского
		 компонента.


		 ![](../../Property_Image.gif)
		 [ParentFont](../IControl/IControl.ParentFont.htm)
		 Свойство ParentFont
		 определяет, будет ли для компонента использоваться шрифт родительского
		 компонента.


		 ![](../../Property_Image.gif)
		 [ParentShowHint](../IControl/IControl.ParentShowHint.htm)
		 Свойство ParentShowHint
		 определяет условие отображения всплывающей подсказки.


		 ![](../../Property_Image.gif)
		 [PopupMenu](../IControl/IControl.PopupMenu.htm)
		 Свойство PopupMenu
		 определяет контекстное меню, которое будет появляться по щелчку
		 правой кнопки мыши на компоненте.


		 ![](../../Property_Image.gif)
		 [Scrolls](../IControl/IControl.Scrolls.htm)
		 Свойство Scrolls возвращает
		 параметры полос прокрутки компонента.


		 ![](../../Property_Image.gif)
		 [ShowHint](../IControl/IControl.ShowHint.htm)
		 Свойство ShowHint определяет,
		 будет ли показываться всплывающая подсказка при задержке курсора
		 мыши над компонентом.


		 ![](../../Property_Image.gif)
		 [TabOrder](../IControl/IControl.TabOrder.htm)
		 Свойство TabOrder определяет
		 позицию компонента в последовательности табуляции.


		 ![](../../Property_Image.gif)
		 [TabStop](../IControl/IControl.TabStop.htm)
		 Свойство TabStop определяет
		 признак необходимости компоненту получать фокус при нажатии кнопки
		 «Tab».


		 ![](../../Property_Image.gif)
		 [Text](../IControl/IControl.Text.htm)
		 Свойство Text определяет
		 строку, идентифицирующую компонент для пользователя.


		 ![](../../Property_Image.gif)
		 [Top](../IControl/IControl.Top.htm)
		 Свойство Top определяет
		 координату верхнего края компонента в пикселях.


		 ![](../../Property_Image.gif)
		 [Visible](../IControl/IControl.Visible.htm)
		 Свойство Visible определяет
		 видимость компонента во время выполнения формы.


		 ![](../../Property_Image.gif)
		 [Width](../IControl/IControl.Width.htm)
		 Свойство Width определяет
		 горизонтальный размер компонента в пикселях.


## Свойства, унаследованные от [IComponent](../IComponent/IComponent.htm)


		 Имя свойства
		 Краткое описание


		 ![](../../Property_Image.gif)
		 [ComponentCount](../IComponent/IComponent.ComponentCount.htm)
		 Свойство ComponentCount
		 возвращает количество дочерних компонентов, расположенных на родительском
		 компоненте.


		 ![](../../Property_Image.gif)
		 [Components](../IComponent/IComponent.Components.htm)
		 Свойство Components
		 возвращает дочерний компонент, индекс которого передается посредством
		 входного параметра.


		 ![](../../Property_Image.gif)
		 [Data](../IComponent/IComponent.Data.htm)
		 Свойство Data предназначено
		 для хранения любых пользовательских данных.


		 ![](../../Property_Image.gif)
		 [Name](../IComponent/IComponent.Name.htm)
		 Свойство Name определяет
		 наименование компонента.


		 ![](../../Property_Image.gif)
		 [Tag](../IComponent/IComponent.Tag.htm)
		 Свойство Tag не используется
		 компилятором. Пользователь может изменить значение данного свойства
		 и использовать его по своему усмотрению.


## Методы


		  Имя метода
		 Краткое описание


		 ![](../../Sub_Image.gif)
		 [AddRow](ICtrlGroupConditions.AddRow.htm)
		 Метод AddRow добавляет
		 элемент.


		 ![](../../Sub_Image.gif)
		 [CanGroup](ICtrlGroupConditions.CanGroup.htm)
		 Метод CanGroup определяет
		 возможность группировки элементов.


		 ![](../../Sub_Image.gif)
		 [CanUngroup](ICtrlGroupConditions.CanUngroup.htm)
		 Метод CanUngroup определяет
		 возможность разгруппировки группы элементов.


		 ![](../../Sub_Image.gif)
		 [GetControl](ICtrlGroupConditions.GetControl.htm)
		 Метод GetControl возвращает
		 визуальный компонент, добавленный в ячейку элемента.


		 ![](../../Sub_Image.gif)
		 [Group](ICtrlGroupConditions.Group.htm)
		 Метод Group группирует
		 отмеченные элементы.


		 ![](../../Sub_Image.gif)
		 [GroupByIndexes](ICtrlGroupConditions.GroupByIndexes.htm)
		 Метод GroupByIndexes
		 группирует элементы по индексам.


		 ![](../../Sub_Image.gif)
		 [InsertRow](ICtrlGroupConditions.InsertRow.htm)
		 Метод InsertRow добавляет
		 элемент по указанному индексу.


		 ![](../../Sub_Image.gif)
		 [Ungroup](ICtrlGroupConditions.Ungroup.htm)
		 Метод Ungroup разгруппировывает
		 отмеченную группу элементов.


		 ![](../../Sub_Image.gif)
		 [UngroupByIndexes](ICtrlGroupConditions.UngroupByIndexes.htm)
		 Метод UngroupByIndexes
		 разгруппировывает группу элементов по индексам.


## Методы, унаследованные от [IListView](../IListView/IListView.htm)


		 Имя метода
		 Краткое описание


		 ![](../../Sub_Image.gif)
		 [AdjustWidth](../IListView/IListView.AdjustWidth.htm)
		 Метод AdjustWidth осуществляет
		 автоматический подгон ширины столбца с учётом его содержимого.


		 ![](../../Sub_Image.gif)
		 [GetItemAt](../IListView/IListView.GetItemAt.htm)
		 Метод GetItemAt позволяет
		 получить элемент компонента в заданной точке.


		 ![](../../Sub_Image.gif)
		 [HitTest](../IListView/IListView.HitTest.htm)
		 Метод HitTest является
		 расширением метода [GetItemAt](../IListView/IListView.GetItemAt.htm).


		 ![](../../Sub_Image.gif)
		 [InsertControl](../IListView/IListView.InsertControl.htm)
		 Метод InsertControl
		 осуществляет вставку компонента в область элементов компонента.


## Методы, унаследованные от [IControl](../IControl/IControl.htm)


		  Имя метода
		 Краткое описание


		 ![](../../Sub_Image.gif)
		 [ClientToScreen](../IControl/IControl.ClientToScreen.htm)
		 Метод ClientToScreen
		 преобразовывает координаты точки, указанные относительно системы
		 координат компонента, в экранные координаты.


		 ![](../../Sub_Image.gif)
		 [DoDragDrop](../IControl/IControl.DoDragDrop.htm)
		 Метод DoDragDrop позволяет
		 начать операцию перетаскивания.


		 ![](../../Sub_Image.gif)
		 [GetImage](../IControl/IControl.GetImage.htm)
		 Метод GetImage возвращает
		 графическое изображение компонента со всеми дочерними компонентами.


		 ![](../../Sub_Image.gif)
		 [ScreenToClient](../IControl/IControl.ScreenToClient.htm)
		 Метод ScreenToClient
		 преобразовывает экранные координаты точки в координаты, указываемые
		 относительно системы координат компонента.


		 ![](../../Sub_Image.gif)
		 [SetFocus](../IControl/IControl.SetFocus.htm)
		 Метод SetFocus устанавливает
		 фокус на данный компонент.


См. также:


[Интерфейсы сборки Forms](../ModForms_Interfaces.htm)


		Справочная
		 система на версию 10.9
		 от 18/08/2025,
		 © ООО «ФОРСАЙТ»,
