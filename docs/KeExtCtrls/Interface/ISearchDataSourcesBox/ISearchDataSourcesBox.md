...
Интерфейс ISearchDataSourcesBox
 содержит свойства и методы компонента среды разработки - [SearchDataSourcesBox](UiDevEnv.chm::/02_Components_constructor_forms/02_Additional_components/SearchDataSourcesBox.htm).
...
## Свойства
		 ...
		 ![](../../Property_Image.gif)
		 [IsAllDataLoaded](ISearchDataSourcesBox.IsAllDataLoaded.htm)
		 Свойство IsAllDataLoaded
		 возвращает флаг, указывающий все ли элементы с результатами поиска
		 загружены в область компонента.


		 ![](../../Property_Image.gif)
		 [Items](ISearchDataSourcesBox.Items.htm)
...
## Методы
		 ...
		 ![](../../Sub_Image.gif)
		 [CancelSearch](ISearchDataSourcesBox.CancelSearch.htm)
		 Метод CancelSearch
		 прерывает процесс BI-поиска.


		 ![](../../Sub_Image.gif)
		 [LoadNextPage](ISearchDataSourcesBox.LoadNextPage.htm)
		 Метод LoadNextPage
		 загружает в область компонента следующий набор элементов с результатами
		 поиска.


		 ![](../../Sub_Image.gif)
		 [StartSearch](ISearchDataSourcesBox.StartSearch.htm)
