...
Интерфейс IDBListBox содержит
 свойства и методы компонента среды разработки - [DBListBox](UiDevEnv.chm::/02_Components_constructor_forms/03_Components_of_the_access_to_data/DBListBox.htm).
...
## Свойства
		 ...
		 ![](../../Property_Image.gif)
		 [DataField](IDBListBox.DataField.htm)
		 Свойство DataField
		 определяет имя поля в источнике данных, с которым связан компонент.


		 ![](../../Property_Image.gif)
		 [DataSource](IDBListBox.DataSource.htm)
		 Свойство DataSource
		 связывает компонент [DBListBox](UiDevEnv.chm::/02_Components_constructor_forms/03_Components_of_the_access_to_data/DBListBox.htm)
		 с набором данных, который содержит поле [DataField](IDBListBox.DataField.htm).


		 ![](../../Property_Image.gif)
		 [Field](IDBListBox.Field.htm)
		 Свойство Field возвращает
		 параметры поля, с которым связан компонент.


		 ![](../../Property_Image.gif)
		 [ReadOnly](IDBListBox.ReadOnly.htm)
		 Свойство ReadOnly определяет,
		 будет ли пользователь иметь возможность изменять значение поля
		 текущей записи.


## Свойства, унаследованные от [IListBox](ModForms.chm::/Interface/IListBox/IListBox.htm)
		 ...
		 ![](../../Property_Image.gif)
		 [BorderStyle](ModForms.chm::/Interface/IListBox/IListBox.BorderStyle.htm)
		 Свойство BorderStyle
		 определяет вид (стиль) границы компонента.
		 ...
		 ![](../../Property_Image.gif)
		 [IntegralHeight](ModForms.chm::/Interface/IListBox/IListBox.IntegralHeight.htm)
		 ...
		 ![](../../Property_Image.gif)
		 [ItemHeight](ModForms.chm::/Interface/IListBox/IListBox.ItemHeight.htm)
		 Свойство ItemHeight
		 возвращает высоту элементов (строк) списка.


		 ![](../../Property_Image.gif)
		 [ItemIndex](ModForms.chm::/Interface/IListBox/IListBox.ItemIndex.htm)
		 Свойство ItemIndex
		 определяет индекс последнего, выделенного элемента компонента
		 [DBListBox](UiDevEnv.chm::/02_Components_constructor_forms/03_Components_of_the_access_to_data/DBListBox.htm).


		 ![](../../Property_Image.gif)
		 [Items](ModForms.chm::/Interface/IListBox/IListBox.Items.htm)
		 Свойство Items определяет
		 список строк компонента [DBListBox](UiDevEnv.chm::/02_Components_constructor_forms/03_Components_of_the_access_to_data/DBListBox.htm).
		 ...
		 в общем списке элементов компонента [DBListBox](UiDevEnv.chm::/02_Components_constructor_forms/03_Components_of_the_access_to_data/DBListBox.htm).


		 ![](../../Property_Image.gif)
		 [Sorted](ModForms.chm::/Interface/IListBox/IListBox.Sorted.htm)
