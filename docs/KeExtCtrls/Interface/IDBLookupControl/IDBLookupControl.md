...
Интерфейс IDBLookupControl является
 базовым для компонентов [DBLookupComboBox](UiDevEnv.chm::/02_Components_constructor_forms/03_Components_of_the_access_to_data/DBLookupComboBox.htm)
 и [DBLookupListBox](UiDevEnv.chm::/02_Components_constructor_forms/03_Components_of_the_access_to_data/DBLookupListBox.htm).
...
## Свойства
		 ...
		 ![](../../Property_Image.gif)
		 [DataField](IDBLookupControl.DataField.htm)
		 Свойство DataField
		 ...
		 ![](../../Property_Image.gif)
		 [DataSource](IDBLookupControl.DataSource.htm)
		 Свойство DataSource
		 связывает компонент группы «Доступ к данным» с набором данных,
		 который содержит поле [DataField](IDBLookupControl.DataField.htm).


		 ![](../../Property_Image.gif)
		 [Field](IDBLookupControl.Field.htm)
		 Свойство Field возвращает
		 параметры поля, с которым связан компонент.


		 ![](../../Property_Image.gif)
		 [KeyField](IDBLookupControl.KeyField.htm)
		 Свойство KeyField определяет
		 поле из набора данных [ListSource](IDBLookupControl.ListSource.htm),
		 значение которого должно соответствовать значению поля [DataField](IDBLookupControl.DataField.htm).


		 ![](../../Property_Image.gif)
		 [KeyValue](IDBLookupControl.KeyValue.htm)
		 Свойство KeyValue определяет
		 значение ключевого поля в активной строке источника данных.


		 ![](../../Property_Image.gif)
		 [ListField](IDBLookupControl.ListField.htm)
		 Свойство ListField
		 ...
		 ![](../../Property_Image.gif)
		 [ListSource](IDBLookupControl.ListSource.htm)
		 Свойство ListSource
		 определяет источник данных для компонента поиска, то есть источник,
		 содержащий поля [KeyField](IDBLookupControl.KeyField.htm)
		 и [ListField](IDBLookupControl.ListField.htm).


		 ![](../../Property_Image.gif)
		 [ReadOnly](IDBLookupControl.ReadOnly.htm)
