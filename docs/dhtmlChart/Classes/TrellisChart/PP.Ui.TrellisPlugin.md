...
Диаграмма треллис подключается в качестве [плагина](UiAdhoc.chm::/Blocks/Plugins/Plugins.htm#web).


## Синтаксис


PP.initClass(PP.Ui.TrellisPlugin, [PP.Ui.Control](dhtmlUi.chm::/Classes/control/control.htm),
 "PP.Ui.TrellisPlugin");
...
## Конструктор
			 ...
			 ![](../../Sub_Image.gif)
			 TrellisPlugin
			 Конструктор TrellisPlugin
			 создает экземпляр компонента TrellisPlugin.
...
## Методы
		 ...
		 ![](../../Sub_Image.gif)
		 getPublicLegend
		 Метод getPublicLegend возвращает
		 компонент общей легенды.
