...
Позволяет выяснить статистическую значимость подмножества факторов,
 участвующих в регрессии.


![](RedundantVariables.gif)
...
Параметры теста:
	...
	- Избыточные ряды. Факторы,
	 для которых проверяется гипотеза о незначимости для регрессионной
	 модели. Избыточным рядом может быть фактор модели, если он отмечен
	 флажком в списке объясняющих рядов;
...
Для каждой статистики приводится начальное
 ...
 отвергается гипотеза о незначимости исключаемых рядов;


	- коэффициенты модели.
	 Коэффициенты регрессии, рассчитанные при отмеченных факторах, которые
	 не выбраны как избыточные. Если в модели используется авторегрессия
