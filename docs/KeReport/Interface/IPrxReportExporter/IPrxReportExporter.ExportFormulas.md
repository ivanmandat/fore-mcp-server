...
Свойство ExportFormulas определяет
 необходимость экспорта формул, содержащихся в таблице. Если значение свойства
 True, то таблица экспортируется
 с формулами, при False экспортируются
 только значения.
...
После выполнения примера лист регламентного отчета будет экспортирован
 в файл формата «XLS». Формулы,
 содержащиеся в таблице, также будут экспортированы.
