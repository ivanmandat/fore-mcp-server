...
Метод StartBatchCommand начинает
...
После вызова данного метода при каждом вызове метода ExportToFile будет
 происходить экспорт отчета, определенного в свойстве [Report](IPrxReportExporter.Report.htm).
 Экспорт будет производиться в файл с параметрами, указанными в методе
 StartBatchCommand. Для завершения
 пакетного экспорта данных необходимо вызвать метод [FinishBatchCommand](IPrxReportExporter.FinishBatchCommand.htm).
