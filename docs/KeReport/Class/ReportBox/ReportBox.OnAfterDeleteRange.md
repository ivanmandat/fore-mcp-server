...
OnAfterDeleteRange(Sender: Object; Args: [IReportRangeDeleteEventArgs](../../Interface/IReportRangeDeleteEventArgs/IReportRangeDeleteEventArgs.htm));
...
Событие OnAfterDeleteRange наступает
 после удаления диапазонов/строк/столбцов.


## Комментарии


Перед удалением диапазонов/строк/столбцов наступает событие [ReportBox.OnBeforeDeleteRange](ReportBox.OnBeforeDeleteRange.htm).
...
В результате выполнения примера при каждом добавлении диапазона/строк/столбцов
 в таблицу в окно консоли будет выводиться сообщение с информацией:
	...
	- лист, с которого были удалены диапазоны/строки/столбцы.
