...
Компонент ControlBar представляет
 собой панель элементов управления регламентного отчета.


Компонент входит в состав компонента [DataArea](../DataArea/DataArea.htm).


Панель элементов управления отображается, если в отчете источнике добавлены
 элементы управления.


Чтобы вернуть элемент управления по его позиции, используйте метод [ControlBarView.getControl](../../../Classes/RegularReport/ControlBarView/ControlBarView.getControl.htm).
 Элемент управления в зависимости от его типа (редактор даты, переключатель
 и т.д.) реализован одним из наследников класса ControlView.


Основное событие компонента - [изменение
 отметки в элементах управления](../../../Classes/RegularReport/ControlBarView/ControlBarView.PropertyChanged.htm).


## Комментарии


Компонент реализован классом [ControlBar](../../../Classes/RegularReport/ControlBar/ControlBar.htm).


## Пример


Компонент ControlBar может иметь
 следующий вид:
...
[Пример размещения компонента ControlBar](ControlBar_Example.htm)
...
## Конструктор
			 ...
			 ![](../../../Sub_Image.gif)
			 [ControlBar](dhtmlReport.chm::/classes/regularreport/controlbar/constructor_controlbar.htm)
			 Конструктор ControlBar
			 создает экземпляр класса ControlBar.


## Свойства, унаследованные от класса [ControlBarView](../../../Classes/RegularReport/ControlBarView/ControlBarView.htm)
		 ...
		 ![](../../../Property_Image.gif)
		 [ControlsRenderState](dhtmlReport.chm::/Classes/RegularReport/ControlBarView/ControlBarView.ControlsRenderState.htm)
		 ...
		 ![](../../../Property_Image.gif)
		 [Orientation](dhtmlReport.chm::/Classes/RegularReport/ControlBarView/ControlBarView.Orientation.htm)
		 Свойство Orientation
		 определяет ориентацию панели элементов управления регламентного
		 отчета.


		 ![](../../../Property_Image.gif)
		 [Source](dhtmlReport.chm::/classes/regularreport/controlbarview/controlbarview.source.htm)
		 Свойство Source определяет
		 регламентный отчет-источник.


## Методы, унаследованные от класса [ControlBarView](../../../Classes/RegularReport/ControlBarView/ControlBarView.htm)
		 ...
		 ![](../../../Sub_Image.gif)
		 [add](dhtmlReport.chm::/Classes/RegularReport/ControlBarView/ControlBarView.add.htm)
		 Метод add добавляет
		 элемент управления на панель.


		 ![](../../../Sub_Image.gif)
		 [clear](dhtmlReport.chm::/Classes/RegularReport/ControlBarView/ControlBarView.clear.htm)
		 Метод clear очищает
		 панель элементов регламентного отчета.


		 ![](../../../Sub_Image.gif)
		 [getControl](dhtmlReport.chm::/classes/regularreport/controlbarview/controlbarview.getcontrol.htm)
		 Метод getControl возвращает
		 элемент управления регламентного отчета по позиции.


		 ![](../../../Sub_Image.gif)
		 [getControls](dhtmlReport.chm::/Classes/RegularReport/ControlBarView/ControlBarView.getControls.htm)
		 Метод getControls возвращает
		 массив элементов управления регламентного отчета.


		 ![](../../../Sub_Image.gif)
		 [getSheetControls](dhtmlReport.chm::/Classes/RegularReport/ControlBarView/ControlBarView.getSheetControls.htm)
		 Метод getSheetControls
		 возвращает массив элементов управления для указанного листа отчета.


		 ![](../../../Sub_Image.gif)
		 [refresh](dhtmlReport.chm::/classes/regularreport/controlbarview/controlbarview.refresh.htm)
		 Метод refresh используется
		 для выборочного обновления панели элементов управления.


		 ![](../../../Sub_Image.gif)
		 [refreshAll](dhtmlReport.chm::/classes/regularreport/controlbarview/controlbarview.refreshall.htm)
		 Метод refreshAll обновляет
		 всю панель элементов управления.


		 ![](../../../Sub_Image.gif)
		 [remove](dhtmlReport.chm::/Classes/RegularReport/ControlBarView/ControlBarView.remove.htm)
		 Метод remove удаляет
		 элемент регламентного отчета по ключу.


		 ![](../../../Sub_Image.gif)
		 [save](dhtmlReport.chm::/Classes/RegularReport/ControlBarView/ControlBarView.save.htm)
		 Метод save сохраняет
		 измерения в элементе управления регламентного отчета.


## События, унаследованные от класса [ControlBarView](../../../Classes/RegularReport/ControlBarView/ControlBarView.htm)
		 ...
		 ![](../../../Event_Image.gif)
		 [PropertyChanged](dhtmlReport.chm::/classes/regularreport/controlbarview/controlbarview.propertychanged.htm)
		 Событие PropertyChanged
		 наступает при смене значений свойств панели элементов управления
