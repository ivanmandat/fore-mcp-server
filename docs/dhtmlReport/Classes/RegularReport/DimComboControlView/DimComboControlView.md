...
Класс DimComboControlView реализует
 представление раскрывающегося списка элементов измерений, который является
 элементом управления регламентного отчета.


## Синтаксис


PP.initClass(PP.Prx.Ui.DimComboControlView, PP.Prx.Ui.[PrxControl](../PrxControl/PrxControl.htm), "DimComboControlView");
