...
PP.initClass(PP.Prx.Report, PP.Mb.[Object](dhtmlMetabase.chm::/Classes/Metabase/Object/Object.htm),
 "Report");
...
## Свойства
		 ...
		 ![](../../../Property_Image.gif)
		 [ActiveSheetKey](Report.ActiveSheetKey.htm)
		 Свойство ActiveSheetKey
		 определяет ключ для открытого листа отчета.


		 ![](../../../Property_Image.gif)
		 [Control](Report.Control.htm)
		 Свойство Control определяет
		 модель элемента управления для регламентного отчета.


		 ![](../../../Property_Image.gif)
		 [EnabledRanges](Report.EnabledRanges.htm)
		 Свойство EnabledRanges
		 определяет диапазоны, доступные для редактирования.


		 ![](../../../Property_Image.gif)
		 [ForceShowControls](Report.ForceShowControls.htm)
		 Свойство ForceShowControls
		 определяет, отображается ли панель «Элементы управления» в отчете,
		 не содержащем элементы управления.


		 ![](../../../Property_Image.gif)
		 [IsDataChanged](Report.IsDataChanged.htm)
		 Свойство IsDataChanged
		 определяет, изменялось ли содержимое ячеек таблицы регламентного
		 отчета.


		 ![](../../../Property_Image.gif)
		 [Params](Report.Params.htm)
		 Свойство Params
		 задает параметры регламентного отчета.
...
## Методы
		 ...
		 ![](../../../Sub_Image.gif)
		 [getActiveSheetIndex](Report.getActiveSheetIndex.htm)
		 Метод getActiveSheetIndex
		 возвращает индекс открытого листа отчета.


		 ![](../../../Sub_Image.gif)
		 [getControls](Report.getControls.htm)
		 Метод getControls возвращает
		 список [элементов управления](../Control/Control.htm)
		 для указанного листа отчета.


		 ![](../../../Sub_Image.gif)
		 [getSheetIndexByKey](Report.getSheetIndexByKey.htm)
		 Метод getSheetIndexByKey
		 возвращает индекс листа по ключу с учетом скрытых листов.


		 ![](../../../Sub_Image.gif)
		 [getSheets](Report.getSheets.htm)
		 Метод getSheets
		 возвращает массив описаний листов отчета.


		 ![](../../../Sub_Image.gif)
		 [getTabId](Report.getTabId.htm)
		 Метод getTabId возвращает
		 строку моникёра для таблицы активного листа.


		 ![](../../../Sub_Image.gif)
		 [getDisplayTabs](Report.getDisplayTabs.htm)
		 Метод getDisplayTabs
		 возвращает признак видимости вкладок листов.


		 ![](../../../Sub_Image.gif)
		 [isAnyControlVisible](Report.isAnyControlVisible.htm)
		 Метод isAnyControlVisible
		 определяет наличие элементов управления для указанного листа отчета.


		 ![](../../../Sub_Image.gif)
		 [isAnyTableSheetExists](Report.isAnyTableSheetExists.htm)
		 Метод isAnyTableSheetExists
		 определяет, имеются ли в отчете таблицы.


## Свойства, унаследованные от класса [Object](dhtmlMetabase.chm::/Classes/Metabase/Object/Object.htm)
...
## Методы, унаследованные от класса [Object](dhtmlMetabase.chm::/Classes/Metabase/Object/Object.htm)
