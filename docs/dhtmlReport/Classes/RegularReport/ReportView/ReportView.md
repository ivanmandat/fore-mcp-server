...
Класс ReportView реализует представление регламентного отчета.


## Комментарии


Является базовым классом для [ReportBox](../ReportBox/ReportBox.htm).


## Синтаксис


PP.initClass(PP.Prx.Ui.ReportView, PP.Ui.[Control](dhtmlUi.chm::/Classes/control/control.htm), "ReportView", PP.Prx.Ui.IReportView);
